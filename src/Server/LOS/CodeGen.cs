...
namespace Server.LOS
{
	...
	{
		internal int m_nelements;
		internal int m_center;

		internal int m_nbytes;
		internal int m_tail;
...
//            1LU << 32,
		...
		public void Execute()
		{
			m_nbytes = m_nelements / 32;

			if (m_nelements % 32 > 0) m_nbytes++;

			m_tail = m_nbytes * 32 - m_nelements;

			m_center = m_nelements / 2;
		...
		public void PrintShadowMakers(char[][,] shadowmask)
		{
			...
			Console.WriteLine("//------------------------------------------------------------------------------");
			Console.WriteLine("//  LOS shadow mask for visibility fields " + m_nelements + " cells square.");
			Console.WriteLine("//  Note: the individual visibility cells are bits in each byte.");
			Console.WriteLine("//------------------------------------------------------------------------------");
			Console.WriteLine("internal static ShadowMaker[,] /*[{0},{0}]*/ m_ShadowMakers = ", m_nelements);
			Console.WriteLine("{");

			for (int n = 0; n < m_nelements * m_nelements; n++)
			{
				//int[,] bytemask = new int[m_nbytes,m_nelements];
				ulong[,] bytemask = new ulong[m_nelements, m_nbytes];
				for (int i = 0; i < m_nelements; i++)
				{
					for (int j = 0; j < m_nelements; j++)
					{
				...
				Console.WriteLine("//  for an occlusion at [" + (int)(n / m_nelements) + "," + n % m_nelements + "]:");

				int masklines = 0;
				for (int i = 0; i < m_nelements; i++)
					for (int j = 0; j < m_nbytes; j++)
				...
				if (masklines == 0)
				{
					...
					for (int i = 0; i < m_nelements; i++)
					{
						...
						if (i == m_nelements - 1) Console.WriteLine("    },");
					}

				if (n % m_nelements == m_nelements - 1)
					Console.WriteLine("  },");
			}
			Console.WriteLine("};");
		...
		{
			Console.WriteLine("//------------------------------------------------------------------------------");
			Console.WriteLine("//  LOS shadow testers for visibility fields " + m_nelements + " cells square.");
			Console.WriteLine("//------------------------------------------------------------------------------");
			Console.WriteLine("internal static ShadowTester[/* {0} */] m_ShadowTesters = ", m_nelements);
			Console.WriteLine("{");
		...
		public void PrintByteArray(char[][,] shadowmask)
		{
			...
			Console.WriteLine("//  Note: the individual visibility cells are bits in each byte.");
			Console.WriteLine("//------------------------------------------------------------------------------");
			Console.WriteLine("shadows [{0},{0}] shadows = ", m_nelements);
			Console.WriteLine("{");
			...
			for (int n = 0; n < m_nelements * m_nelements; n++)
			{
				...
				Console.WriteLine("//  Obstruction at elem[" + (int)(n / m_nelements) + "][" + n % m_nelements + "]:");
			...
			for (int n = 0; n < m_nelements * m_nelements; n++)
			{
				...
				for (int i = 0; i < m_nelements; i++)
				{
					...
					for (int j = 0; j < m_nelements; j++)
					{
						...
						if (j != m_nelements - 1) Console.Write(",");
					}
					if (i != m_nelements - 1) Console.WriteLine("},");
					else Console.WriteLine("}},");
				}
		...
		public void InitMask(char[,] shadowmask, char marker)
		{
			...
			shadowmask[m_center, m_center] = 'O';
		}
		//------------------------------------------------------------------------------
		public void TestSumming()
		{
			...
			char[,] summary = new char[m_nelements, m_nelements];
			char[,] mask1 = new char[m_nelements, m_nelements];
			char[,] mask2 = new char[m_nelements, m_nelements];
			char[,] mask3 = new char[m_nelements, m_nelements];
		...
		{
			char[][,] shadowmask = new char[m_nelements * m_nelements][,];

			for (int i = 0; i < shadowmask.Length; i++)
			{
				shadowmask[i] = new char[m_nelements, m_nelements];
			}
		...
		public void CalcVis(char[,] shadowmask, int xObstruct, int yObstruct)
		{
			...
			for (int i = 0; i < m_nelements; i++)
				for (int j = 0; j < m_nelements; j++)
					DrawVisLine(shadowmask, m_center, m_center, i, j);
		}
		//------------------------------------------------------------------------------
		public void DrawVisLine(char[,] shadowmask, int x1, int y1, int x2, int y2)
		{
			...
			if (dX >= dY)   // if X is the independent variable
			{
				...
				{
					if ((m_center == xcursor) && (m_center == ycursor)) {; }
					else if (shadowmask[xcursor, ycursor] == 'X') blocked = true;
					else if (!blocked) {; }                   // shadowmask[xcursor][ycursor]='-';
		...
		internal ShadowTester[] m_ShadowTesters =
		{
	delegate ( ulong[,] shadow )
	{
		if( ( shadow[0,0] & 137438953472ul ) == 137438953472ul) return true; else return false;
	},
	delegate ( ulong[,] shadow )
	{
		if( ( shadow[0,0] & 68719476736ul ) == 68719476736ul ) return true; else return false;
	}
};

...
public abstract class BaseAddon : BaseItem, IAddon
{
	...
	public virtual AddonFitResult CouldFit(IPoint3D p, Map map, Mobile from, ref BaseHouse house)
	{
		...
		return (from t in doors select t as BaseDoor into door let doorLoc = door.GetWorldLocation() let doorHeight = door.ItemData.CalcHeight from c in Components let addonLoc = new Point3D(p.X + c.Offset.X, p.Y + c.Offset.Y, p.Z + c.Offset.Z) let addonHeight = c.ItemData.CalcHeight where Utility.InRange(doorLoc, addonLoc, 1) && (addonLoc.Z == doorLoc.Z ||
			(addonLoc.Z + addonHeight > doorLoc.Z && doorLoc.Z + doorHeight > addonLoc.Z)) select doorLoc).Any() ? AddonFitResult.DoorTooClose : AddonFitResult.Valid;
	}
	...
	public static bool CheckHouse(Mobile from, Point3D p, Map map, int height, ref BaseHouse house)
	{
		...
		return house != null && (from == null || house.IsCoOwner(from));
	}

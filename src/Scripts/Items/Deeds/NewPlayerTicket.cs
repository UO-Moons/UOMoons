...
public class NewPlayerTicket : BaseItem
{
	...
	{
		private readonly NewPlayerTicket _mTicket;
		...
		protected override void OnTarget(Mobile from, object targeted)
		{
			...
			else switch (targeted)
			{
				case NewPlayerTicket theirTicket:
				{
					...
					{
						from.SendGump(new InternalGump(from, _mTicket));
						them.SendGump(new InternalGump(them, theirTicket));
					}

					break;
				}
				case Item {ItemId: 0x14F0}:
	...
	private class InternalGump : Gump
	{
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			...
			if (item != null)
			{
				...
				_mFrom.SendLocalizedMessage(number);
				_mFrom.AddToBackpack(item);

				if (item2 != null)
					_mFrom.AddToBackpack(item2);
			}
		}
	}
}

...
public class Sextant : BaseItem
{
	...
	public override void OnDoubleClick(Mobile from)
	{
		...
		if (!Format(from.Location, from.Map, ref xLong, ref yLat, ref xMins, ref yMins, ref xEast,
			    ref ySouth))
			return;
		string location = $"{yLat} {yMins}'{(ySouth ? "S" : "N")}, {xLong} {xMins}'{(xEast ? "E" : "W")}";
		from.LocalOverheadMessage(MessageType.Regular, from.SpeechHue, false, location);
	}

	private static bool ComputeMapDetails(Map map, int x, int y, out int xCenter, out int yCenter, out int xWidth, out int yHeight)
	{

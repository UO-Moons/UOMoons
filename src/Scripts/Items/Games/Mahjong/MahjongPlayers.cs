...
public class MahjongPlayers
{
	...
	{
		return Dealer == mobile && m_InGame[DealerPosition];
	}
	...
	public void CheckPlayers()
	{
		...
		for (int i = 0; i < m_Spectators.Count;)
		{
			...
			if (mobile != null && (mobile.NetState == null || mobile.Deleted))
			{
				m_Spectators.RemoveAt(i);
			}
			else if (mobile != null && (!Game.IsAccessibleTo(mobile) || mobile.Map != Game.Map || !mobile.InRange(Game.GetWorldLocation(), 5)))
			{
				m_Spectators.RemoveAt(i);
	...
	private void AddPlayer(Mobile player, int index, bool sendJoinGame)
	{
		...
		SendLocalizedMessage(DealerPosition == index ? 1062773 : 1062772, player.Name);
	}

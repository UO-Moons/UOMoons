...
public class MomentumStrike : SamuraiMove
{
	...
	public override void OnHit(Mobile attacker, Mobile defender, int damage)
	{
		...
		if (weapon != null)
		{
			...
			targets.AddRange(eable.Cast<Mobile>().Where(m => m != defender && m != attacker && m.CanBeHarmful(attacker, false) && attacker.InLOS(m) && SpellHelper.ValidIndirectTarget(attacker, m)));
			eable.Free();
		}

...
public class NobleSacrificeSpell : PaladinSpell
{
	...
	public override void OnCast()
	{
		if (CheckSequence())
		{
			...
			List<Mobile> targets = (from Mobile m in eable where m is not BaseCreature && (!m.Player || (!m.Criminal && !m.Murderer)) where Caster != m && m.InLOS(Caster) && Caster.CanBeBeneficial(m, false, true) select m).ToList();
			eable.Free();

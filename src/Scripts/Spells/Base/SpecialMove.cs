...
public abstract class SpecialMove
{
	...
	{
		foreach (var moveId in SpellRegistry.SpecialMoves.Select(kvp => kvp.Key).Where(moveId => moveId != -1))
		{
	...
	public static SpecialMove GetCurrentMove(Mobile m)
	{
		...
		if (move is {ValidatesDuringHit: true} && !move.Validate(m))
		{
			ClearCurrentMove(m);
			return null;
		}

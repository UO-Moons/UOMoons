...
public class RevealSpell : MagerySpell
{
	...
	public void Target(IPoint3D p)
	{
		...
		else if (CheckSequence())
		{
			...
			if (map != null)
			{
				...
				targets.AddRange(from Mobile m in eable where m is not Mobiles.ShadowKnight || (m.X == p.X && m.Y == p.Y) where m.Hidden && (m.AccessLevel == AccessLevel.Player || Caster.AccessLevel > m.AccessLevel) && CheckDifficulty(Caster, m) select m);
	...
	{
		private readonly RevealSpell _owner;

...
public abstract class MagerySpell : Spell
{
	...
	public override TimeSpan GetCastDelay()
	{
		...
		return !Core.AOS ? TimeSpan.FromSeconds(0.5 + 0.25 * (int)Circle) : base.GetCastDelay();
	}

	public override TimeSpan CastDelayBase => Core.AOS ? TimeSpan.FromMilliseconds((4 + (int)Circle) * CastDelaySecondsPerTick * 1000) : TimeSpan.FromSeconds((3 + (int)Circle) * CastDelaySecondsPerTick);
}

...
public class BaseRegion : Region
{
	...
	public SpawnEntry[] Spawns
	{
		...
		set
		{
			if (_mSpawns != null)
			{
				for (int i = 0; i < _mSpawns.Length; i++)
					_mSpawns[i].Delete();
			}
	...
	public static string GetRuneNameFor(Region region)
	{
		while (region != null)
		{
			if (region is BaseRegion {_mRuneName: { }} br)
				return br._mRuneName;
	...
	{
		if (region == this || !_mExcludeFromParentSpawns)
			return base.AcceptsSpawnsFrom(region);

		return false;
	}

	private Rectangle3D[] _mRectangles;
	private int[] _mRectangleWeights;
	private int _mTotalWeight;

	private static readonly List<Rectangle3D> MRectBuffer1 = new();
	private static readonly List<Rectangle3D> MRectBuffer2 = new();

	private void InitRectangles()
	{
		if (_mRectangles != null)
			return;

		// Test if area rectangles are overlapping, and in that case break them into smaller non overlapping rectangles
		for (int i = 0; i < Area.Length; i++)
		{
			MRectBuffer2.Add(Area[i]);

			for (int j = 0; j < MRectBuffer1.Count && MRectBuffer2.Count > 0; j++)
			{
				Rectangle3D comp = MRectBuffer1[j];

				for (int k = MRectBuffer2.Count - 1; k >= 0; k--)
				{
					Rectangle3D rect = MRectBuffer2[k];
					...
					if (l1 < r2 && r1 > l2 && t1 < b2 && b1 > t2)
					{
						...
						if (l1 < l2)
						{
							MRectBuffer2.Add(new Rectangle3D(new Point3D(l1, t1, sz), new Point3D(l2, b1, ez)));
						}

						if (r1 > r2)
						{
							MRectBuffer2.Add(new Rectangle3D(new Point3D(r2, t1, sz), new Point3D(r1, b1, ez)));
						}

						if (t1 < t2)
						{
							MRectBuffer2.Add(new Rectangle3D(new Point3D(Math.Max(l1, l2), t1, sz), new Point3D(Math.Min(r1, r2), t2, ez)));
						}

						if (b1 > b2)
						{
							MRectBuffer2.Add(new Rectangle3D(new Point3D(Math.Max(l1, l2), b2, sz), new Point3D(Math.Min(r1, r2), b1, ez)));
						}
					}
				}
			}

			MRectBuffer1.AddRange(MRectBuffer2);
			MRectBuffer2.Clear();
		}

		_mRectangles = MRectBuffer1.ToArray();
		MRectBuffer1.Clear();

		_mRectangleWeights = new int[_mRectangles.Length];
		for (int i = 0; i < _mRectangles.Length; i++)
		{
			Rectangle3D rect = _mRectangles[i];
			int weight = rect.Width * rect.Height;

			_mRectangleWeights[i] = weight;
			_mTotalWeight += weight;
		}
	}

	private static readonly List<int> MSpawnBuffer1 = new();
	private static readonly List<Item> MSpawnBuffer2 = new();
	...
	public Point3D RandomSpawnLocation(int spawnHeight, bool land, bool water, Point3D home, int range)
	{
		...
		for (int i = 0; i < 10; i++) // Try 10 times
		{
			...
			{
				int rand = Utility.Random(_mTotalWeight);

				x = int.MinValue; y = int.MinValue;
				minZ = int.MaxValue; maxZ = int.MinValue;
				for (int j = 0; j < _mRectangleWeights.Length; j++)
				{
					int curWeight = _mRectangleWeights[j];

					if (rand < curWeight)
					{
						Rectangle3D rect = _mRectangles[j];
			...
			switch (SpawnZLevel)
			{
				case SpawnZLevel.Lowest:
				{
					...
					for (int j = 0; j < MSpawnBuffer1.Count; j++)
					{
						int l = MSpawnBuffer1[j];
				...
				{
					int index = Utility.Random(MSpawnBuffer1.Count);
					z = MSpawnBuffer1[index];
			...
			bool ok = true;
			for (int j = 0; j < MSpawnBuffer2.Count; j++)
			{
				Item item = MSpawnBuffer2[j];
				ItemData id = item.ItemData;
	...
	public BaseRegion(XmlElement xml, Map map, Region parent) : base(xml, map, parent)
	{
		...
		if (spawning != null)
		{
			ReadBoolean(spawning, "excludeFromParent", ref _mExcludeFromParentSpawns, false);

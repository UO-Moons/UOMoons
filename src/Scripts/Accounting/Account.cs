...
{
	private readonly Account _mAccount;
...
public class Account : IAccount, IComparable, IComparable<Account>
{
		...
		try
		{
			...
			foreach (var a in Accounts.GetAccounts().OfType<Account>().Where(a => a.Count > 0))
			{
				try
				{
					...
					foreach (Mobile m in a._mMobiles.Where(m => m != null))
					{
						var box = m.FindBankNoCreate();
	...
	private AccessLevel _mAccessLevel;
	private TimeSpan _mTotalGameTime;
	private List<AccountComment> _mComments;
	private List<AccountTag> _mTags;
	private readonly Mobile[] _mMobiles;
	...
	/// </summary>
	public List<AccountComment> Comments => _mComments ??= new List<AccountComment>();

	/// <summary>
	/// List of account tags. Type of contained objects is AccountTag.
	/// </summary>
	public List<AccountTag> Tags => _mTags ??= new List<AccountTag>();
	...
	public TimeSpan TotalGameTime
	{
		get
		{
			...
			return _mTotalGameTime;
		}
		set => _mTotalGameTime = value;
	}
	...
	public static string HashMd5(string phrase)
	{
		...
		_mHashBuffer ??= new byte[256];

		int length = Encoding.ASCII.GetBytes(phrase, 0, phrase.Length > 256 ? 256 : phrase.Length, _mHashBuffer, 0);
		byte[] hashed = _mMd5HashProvider.ComputeHash(_mHashBuffer, 0, length);
	...
	public static string HashSha1(string phrase)
	{
		...
		int length = Encoding.ASCII.GetBytes(phrase, 0, phrase.Length > 256 ? 256 : phrase.Length, _mHashBuffer, 0);
		byte[] hashed = _mSha1HashProvider.ComputeHash(_mHashBuffer, 0, length);
	...
	public Account(string username, string password)
	{
		...
		_mAccessLevel = AccessLevel.Player;

		Created = LastLogin = DateTime.UtcNow;
		_mTotalGameTime = TimeSpan.Zero;

		_mMobiles = new Mobile[7];
	...
	public Account(XmlElement node)
	{
		...
		Enum.TryParse(Utility.GetText(node["accessLevel"], "Player"), true, out _mAccessLevel);
		...
		_mMobiles = LoadMobiles(node);
		_mComments = LoadComments(node);
		_mTags = LoadTags(node);
		...
		if (totalGameTime == TimeSpan.Zero)
		{
			for (var i = 0; i < _mMobiles.Length; i++)
			{
				if (_mMobiles[i] is PlayerMobile m)
					totalGameTime += m.GameTime;
			}
		}
		_mTotalGameTime = totalGameTime;
	...
	public static IPAddress[] LoadAddressList(XmlElement node)
	{
		...
		if (addressList != null)
		{
			...
			foreach (XmlElement ip in addressList.GetElementsByTagName("ip"))
			{
				...
				if (!IPAddress.TryParse(Utility.GetText(ip, null), out IPAddress address)) continue;
	...
	public void Save(XmlTextWriter xml)
	{
		...
		if (_mAccessLevel != AccessLevel.Player)
		{
			xml.WriteStartElement("accessLevel");
			xml.WriteString(_mAccessLevel.ToString());
			xml.WriteEndElement();
		}
		...
		for (var i = 0; i < _mMobiles.Length; ++i)
		{
			Mobile m = _mMobiles[i];

			if (m is not {Deleted: false}) continue;
		...
		if (_mComments is {Count: > 0})
		{
			xml.WriteStartElement("comments");

			for (var i = 0; i < _mComments.Count; ++i)
				_mComments[i].Save(xml);
	...
	public Mobile this[int index]
	{
		get
		{
			if (index < 0 || index >= _mMobiles.Length) return null;
			Mobile m = _mMobiles[index];

			if (m is not {Deleted: true}) return m;
			m.Account = null;
			_mMobiles[index] = null;
		...
		set
		{
			if (index < 0 || index >= _mMobiles.Length) return;
			if (_mMobiles[index] != null)
				_mMobiles[index].Account = null;

			_mMobiles[index] = value;
	...
	{
		return other == null ? 1 : string.Compare(Username, other.Username, StringComparison.Ordinal);
	}

...
namespace Server.Gumps
{
	public class NameChangeGump : Gump
	{
		...
		public NameChangeGump(Mobile from) : base(100, 100)
		{
			...
			AddHtml(153, 135, 304, 113, @"The name you've chosen is currently in use and is no longer available. You must choose a different name before you're able to continue.", (bool)true, (bool)false);
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			...
			if (name != "")
			{
				...
				{
					from.SendMessage(0X22, "That name is unacceptable.");
					from.SendGump(new NameChangeGump(from));
					return;
				}

...
public class HelpInfo
{
	...
	public sealed class CommandListGump : BaseGridGump
	{
		private const int EntriesPerPage = 15;
		private readonly int _mPage;
		private readonly List<CommandInfo> _mList;
			...
			: base(30, 30)
		{
			...
			if (list == null)
			{
				_mList = new List<CommandInfo>();

				foreach (var c in SortedHelpInfo.Where(c => from.AccessLevel >= c.AccessLevel))
				{
					_mList.Add(c);
				}
			}
			...
			AddEntryHtml(160, Center($"Page {_mPage + 1} of {(_mList.Count + EntriesPerPage - 1) / EntriesPerPage}"));
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			...
			switch (info.ButtonID)
			{
				...
				{
					if (_mPage > 0)
						m.SendGump(new CommandListGump(_mPage - 1, m, _mList));

					break;
				}
				case 2:
				{
					if ((_mPage + 1) * EntriesPerPage < SortedHelpInfo.Count)
						m.SendGump(new CommandListGump(_mPage + 1, m, _mList));
				...
				default:
				{
					...
					{
						CommandInfo c = _mList[v];

						if (m.AccessLevel >= c.AccessLevel)
						{
							m.SendGump(new CommandInfoGump(c));
							m.SendGump(new CommandListGump(_mPage, m, _mList));
						}
						else
						{
							m.SendMessage("You no longer have access to that command.");
							m.SendGump(new CommandListGump(_mPage, m, null));
						}
					}
					break;
				}

...
public class ConditionCommand : BaseCommand
{
	...
	public override void ExecuteList(CommandEventArgs e, ArrayList list)
	{
		try
		{
			...
			{
				AddResponse(condition.CheckCondition(list[i])
					? "True - that object matches the condition."
					: "False - that object does not match the condition.");
			}
		}
		catch (Exception ex)
		{
...
public class BringToPackCommand : BaseCommand
{
	...
	public override void Execute(CommandEventArgs e, object obj)
	{
		if (obj is Item item)
		{
			AddResponse(e.Mobile.PlaceInBackpack(item)
				? "The item has been placed in your backpack."
				: "Your backpack could not hold the item.");
		}
	}
}
...
{
	private readonly bool _mInGump;
	...
	public override void Execute(CommandEventArgs e, object obj)
	{
		...
		CommandLogging.WriteLine(from, "{0} {1} {2} {3} \"{4}\"", from.AccessLevel, CommandLogging.Format(from), _mInGump ? "messaging" : "telling", CommandLogging.Format(mob), e.ArgString);
...
public class AddToPackCommand : BaseCommand
{
	...
	public override void ExecuteList(CommandEventArgs e, ArrayList list)
	{
		...
		for (int i = 0; i < list.Count; ++i)
		{
			...
			Container cont = obj switch
			{
				Mobile mobile => mobile.Backpack,
...
public class AliasedSetCommand : BaseCommand
{
	...
	public AliasedSetCommand(AccessLevel level, string command, string name, string value, ObjectTypes objects)
	{
		...
		Supports = objects switch
		{
			ObjectTypes.Items => CommandSupport.AllItems,
			ObjectTypes.Mobiles => CommandSupport.AllMobiles,
			_ => CommandSupport.All
		};
	...
	public override void Execute(CommandEventArgs e, object obj)
	{
		string result = Properties.SetValue(e.Mobile, obj, _mName, _mValue);
...
public class HideCommand : BaseCommand
{
	...
	public override void Execute(CommandEventArgs e, object obj)
	{
		...
		CommandLogging.WriteLine(e.Mobile, "{0} {1} {2} {3}", e.Mobile.AccessLevel, CommandLogging.Format(e.Mobile), _mValue ? "hiding" : "unhiding", CommandLogging.Format(m));
		...
		AddResponse(_mValue ? "They have been hidden." : "They have been revealed.");
	}
}
	...
	public override void Execute(CommandEventArgs e, object obj)
	{
		...
		if (from.AccessLevel > targ.AccessLevel)
		{
			...
			if (fromState != null && targState != null)
			{
				...
				{
					CommandLogging.WriteLine(from, "{0} {1} {2} {3}", from.AccessLevel, CommandLogging.Format(from), _mBan ? "banning" : "kicking", CommandLogging.Format(targ));

					targ.Say("I've been {0}!", _mBan ? "banned" : "kicked");

					AddResponse($"They have been {(_mBan ? "banned" : "kicked")}.");
...
public class TraceLockdownCommand : BaseCommand
{
	...
	public override void Execute(CommandEventArgs e, object obj)
	{
		...
		foreach (var house in BaseHouse.AllHouses.Where(house => house.IsSecure(item) || house.IsLockedDown(item)))
		{

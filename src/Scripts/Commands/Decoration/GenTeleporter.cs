...
public static class GenTeleporter
{
	...
	private static bool FindTeleporter(Map map, Point3D p)
	{
		...
		{
			if (item is Teleporter && !(item is KeywordTeleporter) && item is not SkillTeleporter)
			{
				int delta = item.Z - p.Z;

				if (delta is >= -12 and <= 12)
	...
	private static void CreateTeleporter(Point3D pointLocation, Point3D pointDestination, Map mapLocation, Map mapDestination, bool back)
	{
	...
	private static void CreateTeleporter(int xLoc, int yLoc, int zLoc, int xDest, int yDest, int zDest, Map map, bool back)
	{
		CreateTeleporter(new Point3D(xLoc, yLoc, zLoc), new Point3D(xDest, yDest, zDest), map, map, back);
	}

	private static void CreateTeleporter(int xLoc, int yLoc, int zLoc, int xDest, int yDest, int zDest, Map mapLocation, Map mapDestination, bool back)
	{
		CreateTeleporter(new Point3D(xLoc, yLoc, zLoc), new Point3D(xDest, yDest, zDest), mapLocation, mapDestination, back);
	}

	private static void DestroyTeleporter(int x, int y, int z, Map map)
	{
		Point3D p = new(x, y, z);
		IPooledEnumerable eable = map.GetItemsInRange(p, 0);
		...
		foreach (Item item in eable)
		{
			if (item is Teleporter and not KeywordTeleporter and not SkillTeleporter && item.Z == p.Z)
				m_Queue.Enqueue(item);
		}
	...
	private static void CreateTeleportersMap(Map map)
	{
		// Shame
	...
	public static void CreateTeleportersMap2(Map map)
	{
		// Dungeon of rock
	...
	public static void CreateTeleportersMap3(Map map)
	{
	...
	public static void CreateTeleportersMap4(Map map)
	{
	...
	public static void CreateTeleportersTrammel(Map map)
	{
		// Haven
		CreateTeleporter(3632, 2566, 0, 3632, 2566, 20, map, true);
	}

	public static void CreateTeleportersFelucca(Map map)
	{
		// Star room
		CreateTeleporter(5140, 1773, 0, 5171, 1586, 0, map, false);
	}

	public static int CreateTeleporters()
	{

...
namespace Server.Misc
{
	public class CharacterCreation
	{
		...
		public static void Initialize()
		{
			...
			EventSink.CharacterCreated += EventSink_CharacterCreation;
		}
		...
		private static void EventSink_CharacterCreation(CharacterCreatedEventArgs args)
		{
			...
			if (newChar == null)
			{
				Utility.WriteConsole(ConsoleColor.Red, "Login: {0}: Character creation failed, account full", state);
				return;
			}
			...
			newChar.Hunger = Settings.Configuration.Get<int>("Gameplay", "Hunger");
			newChar.Thirst = Settings.Configuration.Get<int>("Gameplay", "Thirst");
			...
			var city = args.City;
			var map = city.Map == Map.Trammel ? Map.Felucca : city.Map;

			newChar.MoveToWorld(city.Location, map);

			Utility.WriteConsole(ConsoleColor.Green, "Login: {0}: New character being created (account={1})", state, args.Account.Username);
			Utility.WriteConsole(ConsoleColor.DarkGreen, " - Character: {0} (serial={1})", newChar.Name, newChar.Serial);
			Utility.WriteConsole(ConsoleColor.DarkGreen, " - Started: {0} {1} in {2}", city.City, city.Location, city.Map);
		}
		...
		private static void AddBackpack(Mobile m)
		{
			...
			PackItem(new Gold(Settings.Configuration.Get<int>("Gameplay", "StartingGold")));

			if (m.Race != Race.Gargoyle)
				PackItem(new Dagger());
			else
				PackItem(new Dagger());//GargishDagger());
		...
		public static void EventSink_CharacterCreated(CharacterCreatedEventArgs args)
		{
			...
			if (newChar == null)
			{
				Utility.WriteConsole(ConsoleColor.Yellow, "Login: {0}: Character creation failed, account full", state);
				return;
			}
			...
			Utility.WriteConsole(ConsoleColor.Cyan, "Login: {0}: New character being created (account={1})", state, args.Account.Username);
			Utility.WriteConsole(ConsoleColor.Cyan, " - Character: {0} (serial={1})", newChar.Name, newChar.Serial);
			Utility.WriteConsole(ConsoleColor.Cyan, " - Started: {0} {1} in {2}", city.City, city.Location, city.Map.ToString());
		...
		private class BadStartMessage : Timer
		{
			...
			private readonly int _mMessage;
			...
			protected override void OnTick()
			{
				_mMobile.SendLocalizedMessage(_mMessage);
			}
		}

		private static readonly CityInfo MNewHavenInfo = new("New Haven", "The Bountiful Harvest Inn", 3503, 2574, 14, Map.Trammel);
		...
		private static CityInfo GetStartLocation(CharacterCreatedEventArgs args, bool isYoung)
		{
			if (Core.ML)
			{
				...
				return MNewHavenInfo;  //We don't get the client Version until AFTER Character creation
		...
		private static Mobile _mMobile;
		...
		private static void EquipItem(Item item, bool mustEquip = false)
		{
			...
			if (_mMobile != null && _mMobile.EquipItem(item))
				return;

			if (_mMobile != null)
			{
				Container pack = _mMobile.Backpack;
				...
				case SkillName.Tracking:
					{
						if (human || elf)
						{
							var shoes = _mMobile?.FindItemOnLayer(Layer.Shoes);

...
namespace Server.Misc
{
	public class SkillCheck
	{
		...
		public static TimeSpan AntiMacroExpire = TimeSpan.FromMinutes(Settings.Configuration.Get<int>("AntiMacro", "Expire"));
		public static int Allowance = Settings.Configuration.Get<int>("AntiMacro", "Allowance");
		private static int LocationSize = Settings.Configuration.Get<int>("AntiMacro", "LocationSize");

		public static bool GGSActive => true;

		static SkillCheck()
		{
			_AntiMacroCode = Settings.Configuration.Get<bool>("AntiMacro", "Enabled");

			_StatGainDelay = TimeSpan.FromMinutes(Settings.Configuration.Get("Mobiles", "StatGainDelay", Core.ML ? 0.05 : 15));
			_PetStatGainDelay = TimeSpan.FromMinutes(Settings.Configuration.Get("Mobiles", "PetStatGainDelay", 5));

			_PlayerChanceToGainStats = 5;
			_PetChanceToGainStats = 5;

			if (!Settings.Configuration.Get("Mobiles", "EnablePlayerStatTimeDelay", false))
				_StatGainDelay = TimeSpan.FromSeconds(0.5);

			if (!Settings.Configuration.Get("Mobiles", "EnablePetStatTimeDelay", false))

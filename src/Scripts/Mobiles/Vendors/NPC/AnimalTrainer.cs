...
public class AnimalTrainer : BaseVendor
{
	...
	{
		_mSbInfos.Add(new SbAnimalTrainer());
	}
	...
	{
		private readonly AnimalTrainer _mTrainer;
	...
	private class ClaimListGump : Gump
	{
		private readonly AnimalTrainer _mTrainer;
		private readonly Mobile _mFrom;
		private readonly List<BaseCreature> _mList;
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			...
			if (index >= 0 && index < _mList.Count)
				_mTrainer.EndClaimList(_mFrom, _mList[index]);
		}
	}
	...
	private class StableTarget : Target
	{
		...
		protected override void OnTarget(Mobile from, object targeted)
		{
			if (targeted is BaseCreature creature)
				_mTrainer.EndStable(from, creature);
			else if (targeted == from)
				_mTrainer.SayTo(from, 502672); // HA HA HA! Sorry, I am not an inn.
			else
				_mTrainer.SayTo(from, 1048053); // You can't stable that!
		}
	}
	...
	public static bool CanClaim(Mobile from, BaseCreature pet)
	{

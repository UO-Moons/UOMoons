...
public class PlayerMobile : BaseMobile, IHonorTarget
{
	...
	public override void ToggleFlying()
	{
		...
		if (!Flying)
		{
			...
			{
				(Spell as Spell)?.Disturb(DisturbType.Unspecified, false, false);
	...
	private DuelPlayer _mDuelPlayer;
	private string _fameKarmaTitle;
	private string _paperdollSkillTitle;
	private string _subtitleSkillTitle;
	private string _currentChampTitle;
	private string _overheadTitle;
	private int _currentVeteranTitle;
	...
	[CommandProperty(AccessLevel.GameMaster)]
	public ChampionTitleInfo ChampionTitles { get; set; }
	...
	public List<Mobile> AllFollowers { get => _mAllFollowers ??= new List<Mobile>(); }
	public Guilds.RankDefinition GuildRank { get => AccessLevel >= AccessLevel.GameMaster ? Guilds.RankDefinition.Leader : _mGuildRank; set => _mGuildRank = value; }
	[CommandProperty(AccessLevel.GameMaster)]
	...
	[CommandProperty(AccessLevel.GameMaster)]
	public int ToothAche { get => CandyCane.GetToothAche(this); set => CandyCane.SetToothAche(this, value); }
	[CommandProperty(AccessLevel.GameMaster)]
	...
	public string FameKarmaTitle { get => _fameKarmaTitle; set { _fameKarmaTitle = value; InvalidateProperties(); } }
	public string PaperdollSkillTitle { get => _paperdollSkillTitle; set { _paperdollSkillTitle = value; InvalidateProperties(); } }
	public string SubtitleSkillTitle { get => _subtitleSkillTitle; set { _subtitleSkillTitle = value; InvalidateProperties(); } }
	public string CurrentChampTitle { get => _currentChampTitle; set { _currentChampTitle = value; InvalidateProperties(); } }
	public string OverheadTitle { get => _overheadTitle; set { _overheadTitle = value; InvalidateProperties(); } }
	public int CurrentVeteranTitle { get => _currentVeteranTitle; set { _currentVeteranTitle = value; InvalidateProperties(); } }
	...
	private static readonly int[] m_BodyArray = new int[] { 1, 2, 3, 4, 5, 6, 8, 10, 11, 12, 13, 14, 15, 16, 21, 22, 24, 26, 28, 29, 30, 31, 33, 39, 42, 46, 47, 48, 50, 51, 52, 53, 58, 62, 64, 70, 72, 75, 80, 81, 85, 86, 88, 90, 97, 101, 104, 123, 134, 145, 149, 150, 151, 152, 154, 157, 400, 401, 605, 606 };
	...
	public override bool CanBeHarmful(IDamageable damageable, bool message, bool ignoreOurBlessedness, bool ignorePeaceCheck)
	{
		...
		if (target is not BaseCreature {IsInvulnerable: true} && target is not PlayerVendor &&
		...
		if (damageable is IDamageableItem {CanDamage: false})
		{
			if (true)
	...
	private bool CheckInsuranceOnDeath(Item item)
	{
		...
		#region Dueling
		if (_mDuelPlayer != null && DuelContext is {Registered: true, Started: true} && !_mDuelPlayer.Eliminated)
			return true;
		#endregion
	...
	public override void OnDeath(Container c)
	{
		...
		if (Murderer && DateTime.UtcNow >= _mNextJustAward)
		{
			...
			if (m != null && m is PlayerMobile && m != this)
			{
				...
				{// You have gained a path in Justice!// You have gained in Justice.
					m.SendLocalizedMessage(gainedPath ? 1049367 : 1049363);
		...
		if (DuelContext is not {Registered: true, Started: true} || _mDuelPlayer == null || _mDuelPlayer.Eliminated)
	...
	{
		if (Young && (DuelContext is not {Started: true} || DuelContext.Finished))
			return true;
			...
			case 0:
				{
					...
					Profession = (Profession)reader.ReadEncodedInt();
	...
	public override bool CanSee(Mobile m)
	{
		switch (m)
		{
			case IConditionalVisibility visibility when !visibility.CanBeSeenBy(this):
				return false;
			case CharacterStatue statue:
				statue.OnRequestedAnimation(this);
				break;
			case PlayerMobile pm when pm.VisibilityList.Contains(this):
				return true;
		}
		...
		if (m.IsPlayer() && owner is PlayerMobile pmOwner && pmOwner.DuelContext != DuelContext)
		{
			return false;
		}
	...
	{
		if (item is IConditionalVisibility visibility && !visibility.CanBeSeenBy(this))
			return false;
	...
	public bool CheckYoungProtection(Mobile from)
	{
		...
		if (Region is BaseRegion {YoungProtected: false})
			return false;

		if (from is BaseCreature {IgnoreYoungProtection: true})
			return false;

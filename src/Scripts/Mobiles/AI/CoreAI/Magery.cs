...
{
	private static readonly int[] m_MageryMana = { 4, 6, 9, 11, 14, 20, 40, 50 };
	private DateTime m_LastExplosion;

	private static bool CanUseMagerySummon => false; //m_ForceUseAll; }

	private void MageryPower()
	{
	...
	private Spell GetMagerySpell()
	{
		...
		{
			spell = m_Mobile.Skills[SkillName.Magery].Value switch
			{
				> 80.0 => new ManaVampireSpell(m_Mobile, null),
				> 40.0 => new ManaDrainSpell(m_Mobile, null),
				_ => spell
			};
	...
	private Spell CheckCurse()
	{
		...
		Spell spell = whichone switch
		{
			1 => new FeeblemindSpell(m_Mobile, null),
			2 => new WeakenSpell(m_Mobile, null),
			3 => new ClumsySpell(m_Mobile, null),
			_ => null
		};
	...
	private Spell CheckMagerySummon()
	{
		...
		switch (m_Mobile.Skills[SkillName.Magery].Value)
		{
			case > 95.0 when m_Mobile.Mana > 50:
			{
				...
				else if (slots > 2 && whichone < 3)
				{
					...
					spell = whichone switch
					{
						< 4 => new AirElementalSpell(m_Mobile, null),
						< 5 => new EarthElementalSpell(m_Mobile, null),
						_ => new EnergyVortexSpell(m_Mobile, null)
					};

				break;
			}
			// 5th level
			case > 55.0 when m_Mobile.Mana > 14:
			{
	...
	private Spell GetRandomMageryDamageSpell()
	{
		if (m_Mobile.Debug)
		{

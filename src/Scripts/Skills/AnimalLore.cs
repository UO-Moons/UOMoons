...
public class AnimalLoreGump : Gump
{
	private static string FormatSkill(BaseCreature c, SkillName name)
	{
		...
		return skill.Base < 10.0 ? "<div align=right>---</div>" : $"<div align=right>{skill.Value:F1}</div>";
	}

	private static string FormatAttributes(int cur, int max)
	{
		return max == 0 ? "<div align=right>---</div>" : $"<div align=right>{cur}/{max}</div>";
	}
	...
	{
		return val == 0 ? "<div align=right>---</div>" : $"<div align=right>{val:F1}</div>";
	}

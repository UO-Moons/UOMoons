...
public class Hiding
{
	...
	public static TimeSpan OnUse(Mobile m)
	{
		...
		{
			house ??= ((BaseHouse.FindHouseAt(new Point3D(m.X - 1, m.Y, 127), m.Map, 16) ?? BaseHouse.FindHouseAt(new Point3D(m.X + 1, m.Y, 127), m.Map, 16)) ??
			           BaseHouse.FindHouseAt(new Point3D(m.X, m.Y - 1, 127), m.Map, 16)) ?? BaseHouse.FindHouseAt(new Point3D(m.X, m.Y + 1, 127), m.Map, 16);
		...
		bool badCombat = !CombatOverride && m.Combatant != null && m.InRange(m.Combatant.Location, range) && ((Mobile)m.Combatant).InLOS(m.Combatant);

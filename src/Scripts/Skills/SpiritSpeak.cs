...
internal class SpiritSpeak
{
	...
	public static TimeSpan OnUse(Mobile m)
	{
		if (Core.AOS)
		{
			...
			return spell.IsCasting ? TimeSpan.FromSeconds(5.0) : TimeSpan.Zero;
		}
		...
		if (m.CheckSkill(SkillName.SpiritSpeak, 0, 100))
		{
			...
			m.PlaySound(0x24A);
			m.SendLocalizedMessage(502444);//You contact the neither world.
		}
		else
		{
			m.SendLocalizedMessage(502443);//You fail to contact the neither world.
	...
	private class SpiritSpeakTimer : Timer
	{
		...
		protected override void OnTick()
		{
			...
			m_Owner.SendLocalizedMessage(502445);//You feel your contact with the neither world fading.
		}
	}

	private class SpiritSpeakSpell : Spell
	{
		private static readonly SpellInfo MInfo = new("Spirit Speak", "", 269);

		public override bool BlockedByHorrificBeast => false;

		public SpiritSpeakSpell(Mobile caster) : base(caster, null, MInfo)
		{
		}
		...
		public override void OnCast()
		{
			...
			{
				if (item is not Corpse corpse || corpse.Channeled) continue;
				toChannel = corpse;
				break;
			}

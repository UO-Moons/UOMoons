...
namespace Server.Misc
{
	...
	{
		public static bool Enabled { get; } = Settings.Configuration.Get<bool>("Misc", "TestCenterEnabled");
		...
		public static void GiveResources(Mobile from)
		{
			...
			for (var i = 0; i < Imbuing.IngredTypes.Length; i++)
			{
			...
			for (var i = 0; i < Loot.RareGemTypes.Length; i++)
			{
		...
		public static void GiveArtifacts(Mobile from)
		{
			...
			for (var i = 0; i < DoomGauntlet.DoomArtifacts.Length; i++)
			{
			...
			for (var i = 0; i < MondainsLegacy.Artifacts.Length; i++)
			{
		...
		public static void FillBank(Mobile m)
		{
			...
			PlaceItemIn(bag, 65, 107, new NecromancerSpellbook(0xFFFF));
			PlaceItemIn(bag, 85, 107, new BookOfChivalry(0x3FF));

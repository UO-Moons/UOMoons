...
public class PotionOfGloriousFortune : BaseItem
{
	...
	public bool TryAddEffects(Mobile m)
	{
		...
		if (!UnderEffects(m, Type))
		{
			Table.Add(new PotionArray { Mobile = m, Date = DateTime.UtcNow + TimeSpan.FromHours(1), Type = Type });

			m.SendLocalizedMessage(1158719); // You are now under the effect of the Potion of Glorious Fortune.

			BuffInfo.AddBuff(m, new BuffInfo(BuffIcon.PotionGloriousFortune, 1158688, 1158720, TimeSpan.FromMinutes(60), m, GetScaler(Type).ToString(), true));

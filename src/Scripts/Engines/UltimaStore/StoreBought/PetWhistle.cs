...
public class PetWhistle : BaseItem
{
	...
	public override void OnDoubleClick(Mobile from)
	{
		...
		{
			if (from.Account is not Account acct || acct.Username != Account)
			{
	...
	private class LinkBondedPetEntry : ContextMenuEntry
	{
		...
		{
			if (_from == null || _item.Deleted)
				return;

			if (_item.IsChildOf(_from.Backpack))
			{
				_from.SendLocalizedMessage(1159390); // Target a bonded pet to link this whistle.
				_from.Target = new InternalTarget(_item);
			}
			else
			{
				_from.SendLocalizedMessage(1062334); // This item must be in your backpack to be used.
			}
		}
	}
		...
		protected override void OnTarget(Mobile from, object targeted)
		{
			...
			if (targeted is BaseCreature creature && creature.ControlMaster == from)
			{
				if (creature.IsBonded)
				{
					...
					{
						from.SendLocalizedMessage((DateTime.UtcNow - _item.NextLinkedTime).Seconds > 60
							? 1159391
							: 1159392);
					}
				}

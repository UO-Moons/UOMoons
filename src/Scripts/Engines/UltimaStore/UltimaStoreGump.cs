...
public class UltimaStoreGump : BaseGump
{
	...
	public override void AddGumpLayout()
	{
		...
		if (!Search && Category == StoreCategory.Cart)
		{
			...
			if (profile is {Cart.Count: > 0})
			{
				int i = 0;

				foreach (var (entry, amount) in profile.Cart)
				{
			...
			for (int i = listIndex; i < StoreList.Count && pageIndex < 6; i++)
			{
				...
				for (int j = 0; j < entry.Name.Length; j++)
				{
					if (entry.Name[j].Number > 0)
						AddHtmlLocalized(x, y + j * 14 + 4, 183, 25, 1114513,
							$"#{entry.Name[j].Number}", 0x7FFF, false, false);
					else
	...
	public override void OnResponse(RelayInfo info)
	{
		...
		switch (id)
		{
			...
			case 107:
			{
				User.LaunchBrowser(!string.IsNullOrWhiteSpace(Configuration.Website)
					? Configuration.Website
					: "https://uo.com/ultima-store/");
			...
			case 112:
			{
				...
				{
					// re-orders the list
					if (oldSort is SortBy.Newest or SortBy.Oldest)
					{
						ColUtility.Free(StoreList);
...
public class NoFundsGump : BaseGump
{
	...
	public override void AddGumpLayout()
	{
		...
		AddHtml(30, 60, 350, 60, Color("#da0000",
			$"This transaction cannot be completed due to insufficient funds available. Visit your shards website for more information on how to obtain {Configuration.CurrencyName}."), false, false);
	...
	public override void OnResponse(RelayInfo info)
	{
		if (info.ButtonID == 195)
		{
			User.LaunchBrowser(!string.IsNullOrEmpty(Configuration.Website)
				? Configuration.Website
				: "https://uo.com/ultima-store/");
		}

...
public static class UltimaStore
{
	...
	public static void Initialize()
	{
		...
		Register<TransmogrificationPotion>(1159501, 1159496, 0xA1E9, 0, 2741, 700, cat);
	...
	private static string GetStringName(TextDefinition[] text)
	{
		...
		foreach (TextDefinition td in text)
		{
			...
			//}
			// else
			if (!string.IsNullOrWhiteSpace(td.String))
			{
				str += $"{td.String} ";
			}
		}
	...
	public static int GetSubTotal(Dictionary<StoreEntry, int> cart)
	{
		...
		double sub = cart.Aggregate(0.0, (current, kvp) => current + kvp.Key.Cost * kvp.Value);
				...
				for (int i = 0; i < entry.Value; i++)
				{
					if (!entry.Key.Construct(m))
					{
						...
						try
						{
							...
							Utility.WriteConsole(ConsoleColor.Red, "[Ultima Store]: Bad Constructor: {0}", entry.Key.ItemType.Name);
						}
						catch
						{
							// ignored
...
{
	private static readonly List<Item> m_DisplayItems = new();
	...
	private void AddDisplayItem(Item item)
	{
		...
		if (!m_DisplayItems.Contains(item))
		{
	...
	public override void Serialize(GenericWriter writer)
	{
		...
		writer.WriteItemList(m_DisplayItems, true);
	}

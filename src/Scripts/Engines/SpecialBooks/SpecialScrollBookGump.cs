...
public class SpecialScrollBookGump : BaseGump
{
		...
		: base(pm, 150, 200)
	{
		...
		_ = pm.CloseGump(typeof(SpecialScrollBookGump));
	}
	...
	{
		AddHtmlLocalized(0, 15, 175, 20, CenterLoc, $"#{SkillInfo.Table[(int) Skill].Localization}", 0, false, false);
	...
	{
		return Book.Items.FirstOrDefault(i => i is SpecialScroll scroll && skills.Contains(scroll.Skill)) != null;
	}

	public bool HasScroll(SkillName skill)
	{
		return Book.Items.FirstOrDefault(i => i is SpecialScroll scroll && skill == scroll.Skill) != null;
	}

	public int GetTotalScrolls(SkillName skill, double value)
	{
		return Book.Items.OfType<SpecialScroll>().Count(s => s.Skill == skill && Math.Abs(value - s.Value) < 120);
	}
}

...
public class CraftGump : Gump
{
	...
	private readonly CraftPage _mPage;

	private const int LabelHue = 0x480;
	private const int LabelColor = 0x7FFF;
	private const int FontColor = 0xFFFFFF;

	public bool Locked => AutoCraftTimer.HasTimer(_mFrom);
		...
		: base(40, 40)
	{
		...
		#region SA
		// Alter option
		if (Core.SA && _mCraftSystem.CanAlter)
		{
		...
		if (Core.SA)
		{
			...
			AddHtmlLocalized(305, 425, 150, 18, context is {QuestOption: CraftQuestOption.QuestItem} ? 1112534 : 1112533, LabelColor, false, false); // QUEST ITEM
		}
		// ****************************************
		#endregion
		...
		if (Locked && AutoCraftTimer.AutoCraftTable.ContainsKey(_mFrom))
		{
			AutoCraftTimer timer = AutoCraftTimer.AutoCraftTable[_mFrom];
		...
		switch (notice)
		{
			case int notice1 and > 0:
				AddHtmlLocalized(170, 295, 350, 40, notice1, LabelColor, false, false);
				break;
			case string:
		...
		if (craftSystem.CraftSubRes.Init)
		{
			...
			if (from.Backpack != null)
			{
				...
				{
					Item[] items2 = _mFrom.Backpack.FindItemsByType(resourceType2, true);
			...
			if (nameNumber > 0)
			{
				...
				AddHtmlLocalized(50 + (context is {DoNotColor: true} ? 13 : 0), 365, 250, 18, nameNumber, resourceCount.ToString(), LabelColor, false, false);
			}
			else
			{
				AddLabel(50, 362, LabelHue, (context is {DoNotColor: true} ? "*" : "") +
				                            $"{nameString} ({resourceCount} Available)");
			}
		}
		...
		switch (page)
		{
			...
			default:
			{
				if (context is {LastGroupIndex: > -1})
				{
	...
	{
		CraftSubResCol res = (opt ? _mCraftSystem.CraftSubRes2 : _mCraftSystem.CraftSubRes);
		...
		for (int i = 0; i < res.Count; ++i)
		{
			...
			if (index == 0)
			{
				...
				AddHtmlLocalized(255, 260, 200, 18, context is not {DoNotColor: true} ? 1061591 : 1061590, LabelColor, false, false);
			}
			...
			if (from.Backpack != null)
			{
				...
				if (alt != null)
				{
					Item[] items2 = _mFrom.Backpack.FindItemsByType(alt, true);
	...
	public void CreateItemList(int selectedGroup)
	{
		...
		CraftGroupCol craftGroupCol = _mCraftSystem.CraftGroups;
	...
	public void CraftItem(CraftItem item)
	{
		...
		{
			item.TryCraft(_mFrom, item, _mTool);
			return;
		}

		int num = _mCraftSystem.CanCraft(_mFrom, _mTool, item.ItemType);
		...
		else
		{
			...
			if (context != null)
			{
				CraftSubResCol res = (item.UseSubRes2 ? _mCraftSystem.CraftSubRes2 : _mCraftSystem.CraftSubRes);
			...
			_mCraftSystem.CreateItem(_mFrom, item.ItemType, type, _mTool, item);
		}
	}
	...
	public override void OnResponse(NetState sender, RelayInfo info)
	{
		...
		CraftSystem system = _mCraftSystem;
		CraftGroupCol groups = system.CraftGroups;
		CraftContext context = system.GetContext(_mFrom);
		...
		switch (type)
		{
			case 0: // Show group
			{
				...
				if (index >= 0 && index < groups.Count)
				{
					...
					_mFrom.SendGump(new CraftGump(_mFrom, system, _mTool, null));
				}
			...
			case 2: // Item details
			{
				...
				if (groupIndex >= 0 && groupIndex < groups.Count)
				{
					...
					if (index >= 0 && index < group.CraftItems.Count)
					{
						_mFrom.SendGump(new CraftGumpItem(_mFrom, system, group.CraftItems.GetAt(index), _mTool));
					}
				}
			...
			case 4: // Item details (last 10)
			{
				...
				if (index >= 0 && index < lastTen.Count)
				{
					_mFrom.SendGump(new CraftGumpItem(_mFrom, system, lastTen[index], _mTool));
				}
			...
			{
				if (_mPage == CraftPage.PickResource && index >= 0 && index < system.CraftSubRes.Count)
				{
					_ = context?.LastGroupIndex ?? -1;

					CraftSubRes res = system.CraftSubRes.GetAt(index);

					if (_mFrom.Skills[system.MainSkill].Base < res.RequiredSkill)
					{
						_mFrom.SendGump(new CraftGump(_mFrom, system, _mTool, res.Message));
					}
					else
					{
						if (context != null)
				...
				else if (_mPage == CraftPage.PickResource2 && index >= 0 && index < system.CraftSubRes2.Count)
				{
					_ = context?.LastGroupIndex ?? -1;
			...
			case 6: // Misc. buttons
			{
				switch (index)
				{
					case 0: // Resource selection
					{
						...
						{
							_mFrom.SendGump(new CraftGump(_mFrom, system, _mTool, null, CraftPage.PickResource));
						}
					...
					case 1: // Smelt item
					{
						if (system.Resmelt)
						{
							Resmelt.Do(_mFrom, system, _mTool);
						}
					...
					case 2: // Make last
					{
						...
						{
							_mFrom.SendGump(new CraftGump(_mFrom, _mCraftSystem, _mTool, 1044165, _mPage)); // You haven't made anything yet.
						}
					...
					case 4: // Toggle use resource hue
					{
						...
						_mFrom.SendGump(new CraftGump(_mFrom, _mCraftSystem, _mTool, null, _mPage));
					...
					case 5: // Repair item
					{
						if (system.Repair)
						{
							Repair.Do(_mFrom, system, _mTool);
						}
					...
					case 7: // Resource selection 2
					{
						...
						{
							_mFrom.SendGump(new CraftGump(_mFrom, system, _mTool, null, CraftPage.PickResource2));
						}
					...
					case 8: // Enhance item
					{
						...
						{
							Enhance.BeginTarget(_mFrom, system, _mTool);
						}
					...
					case 9: // Alter Item (Gargoyle)
					{
						if (system.CanAlter)
						{
							_mFrom.SendLocalizedMessage(1111867); // You must be near a soulforge to alter an item.
						}
						break;
					}
					case 10: // Quest Item/Non Quest Item toggle
					{
						...
						//break;
						context.QuestOption = context.QuestOption switch
						{
							CraftQuestOption.QuestItem => CraftQuestOption.NonQuestItem,
							CraftQuestOption.NonQuestItem => CraftQuestOption.QuestItem,
							_ => context.QuestOption
						};

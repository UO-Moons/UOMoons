...
public class DefTailoring : CraftSystem
{
	...
	public static CraftSystem CraftSystem => _mCraftSystem ??= new DefTailoring();
	...
	private static readonly Type[] MTailorColorables = {
	...
	public override bool RetainsColorFrom(CraftItem item, Type type)
	{
		...
		for (var i = 0; !contains && i < MTailorColorables.Length; ++i)
			contains = (MTailorColorables[i] == type);
	...
	{
		var index = AddCraft(typeof(CutUpCloth), 1044457, 1044458, 0.0, 0.0, typeof(BoltOfCloth), 1044453, 1, 1044253);
	...
	private void CutUpCloth(Mobile m, CraftItem craftItem, ITool tool)
	{
		...
		Timer.DelayCall(TimeSpan.FromSeconds(Delay), () =>
		{
			...
			else
			{
				...
				foreach (var cloth in bolts.Select(kvp => new UncutCloth(kvp.Value * 50)
				         {
					         Hue = kvp.Key
				         }))
				{
	...
	private void CombineCloth(Mobile m, CraftItem craftItem, ITool tool)
	{
		...
		Timer.DelayCall(TimeSpan.FromSeconds(Delay), () =>
		{
			...
			Dictionary<int, int> cloth = new();
			List<Item> toConsume = new();
			object num = null;

			foreach (var item in from item in pack.Items let t = item.GetType() where t == typeof(UncutCloth) || t == typeof(Cloth) || t == typeof(CutUpCloth) select item)
			{
			...
			else
			{
				...
				foreach (var c in cloth.Select(kvp => new UncutCloth(kvp.Value)
				         {
					         Hue = kvp.Key
				         }))
				{
	...
	private static void DropItem(Mobile from, Item item, ITool tool)
	{
		if (tool is Item {Parent: Container} item1)
		{
			Container cntnr = (Container)item1.Parent;

			if (cntnr.TryDropItem(from, item, false)) return;

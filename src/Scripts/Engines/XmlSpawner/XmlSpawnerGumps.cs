				...
				if (origtext != null && oldtext != null && newtext != null)
				{
					try
					{
						...
						if (firstindex >= 0)
						{
							...
							if (firstindex > 0)
							{
								editedtext = $"{origtext[..firstindex]}{newtext}{origtext.Substring(secondindex, lastindex - secondindex + 1)}";
							}
							else
							{
								editedtext = string.Concat(newtext, origtext.AsSpan(secondindex, lastindex - secondindex + 1));
							}
        ...
        */
		...
		public static void ProcessSpawnerBookEntry(Mobile from, object[] args, string entry)
		{
			...
			else
			{
				...
				_ = m_Spawner.SpawnObjects.Length - 1;
			...
			if (ns != null && ns.Gumps != null)
			{
				...
				foreach (XmlSpawnerGump g in refresh)
				{
					...
					if (g.m_Spawner != null /*&& g.m_Spawner.SpawnerGump == g */)
					{
						...
						XmlSpawnerGump xg = new(g.m_Spawner, g.X, g.Y, g.m_ShowGump, g.xoffset, g.page, g.Rentry);
				...
				default:
					{
								...
								if (info.ButtonID >= 800 && info.ButtonID < 800 + MaxSpawnEntries)
						{
...
#if (BOOKTEXTENTRY)
							// display a new gump
							XmlSpawnerGump newgump = new(m_Spawner, X, Y, m_ShowGump, xoffset, page);
							...
							XmlTextEntryBook book = new(0, string.Empty, m_Spawner.Name, 20, true, new XmlTextEntryBookCallback(ProcessSpawnerBookEntry), args);
						...
						else if (info.ButtonID >= 1300 && info.ButtonID < 1300 + MaxSpawnEntries)// goto spawn buttons
						{
							nclicks++;
						...
						else if (info.ButtonID >= 4000 && info.ButtonID < 4001 + (MaxSpawnEntries / MaxEntriesPerPage))// page buttons
						{
							// which page
							this.page = info.ButtonID - 4000;

						}
						else if (info.ButtonID >= 6000 && info.ButtonID < 6000 + MaxSpawnEntries)// toggle the disabled state of the entry
						{
						...
						else if (info.ButtonID >= 5000 && info.ButtonID < 5000 + MaxSpawnEntries)
						{
							...
							{
								XmlSpawnerGump newg = new(m_Spawner, X, Y, m_ShowGump, xoffset, page);
							...
							else
							{
								...
								XmlSpawnerGump newg = new(m_Spawner, this.X, this.Y, this.m_ShowGump, this.xoffset, this.page, re);

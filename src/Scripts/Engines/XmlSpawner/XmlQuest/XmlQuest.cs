...
namespace Server.Items
{
	...
	public abstract class XmlQuest
	{
		...
		{
			private readonly IXmlQuest m_quest;
		...
		public static void QuestButton(Mobile m)
		{
			if (m == null || m == null)
				return;
			Mobile from = m;
		...
		public static object CreateItem(IEntity from, string action, out string status_str, Type typerestrict)
		{
			...
			XmlSpawner.SpawnObject TheSpawn = new(null, 0)
			{
			...
			{
				if (TheSpawn.SpawnedObjects[0] is Item item)
				{
					return item;
				}
				else if (TheSpawn.SpawnedObjects[0] is Mobile mobile)
				{
					// dont do mobiles as rewards at this point
					mobile.Delete();
				}
		...
		public static void Collect(Mobile m, Item target, IXmlQuest quest)
		{
			...
			if (target != null && !target.Deleted)
			{
				...
				if (quest.PartyEnabled && p != null)
				{
					...
					List<PartyMemberInfo> startlist = new();
					List<PartyMemberInfo> randlist = new();
					...
					foreach (PartyMemberInfo mi in randlist)
					{
						...
						if (quest.PartyRange < 0 || Utility.InRange(m.Location, member.Location, quest.PartyRange))
						{
							...
							if (questitem != null && !questitem.Deleted && questitem is IXmlQuest quest1)
							{
								ApplyCollected(target, quest1);
							}
						}
					}
				}
		...
		public static bool CheckCollectObjective(IXmlQuest quest, Item item, string objectivestr, string statestr, out string newstatestr, out bool collectstatus)
		{
			...
			CheckArgList(arglist, 2, item, out string typestr, out int targetcount, out bool checkprop, out string status_str);
			...
			if (found)
			{
				_ = int.TryParse(statestr, out int current);
		...
		public static bool RegisterGive(Mobile from, Mobile to, Item item)
		{
			...
			{
				List<Item> questlist = FindXmlQuest(mobile);
				if (questlist != null)
				{
					// now go through the list and try to apply the dropped item
					for (int i = 0; i < questlist.Count; i++)
					{
						if (questlist[i] is IXmlQuest quest)
						{
							if (Give(from, to, item, quest))
							{
								found = true;
							}
						}
		...
		public static bool CheckGiveObjective(IXmlQuest quest, Mobile mob, Item item, string objectivestr, string statestr, out string newstatestr, out bool givestatus)
		{
			...
			CheckArgList(arglist, 3, item, out string typestr, out int targetcount, out bool checkprop, out string status_str);
			...
			if (arglist.Length > 2)
			{
				...
				{
					if (item != null && !item.Deleted && (arglist[2] == item.Name) && checkprop && (typestr == null || BaseXmlSpawner.CheckType(item, typestr)))
					{
					}
				}
			}
		...
		public static bool CheckKillObjective(IXmlQuest quest, Mobile m_killed, Mobile m_killer, string objectivestr, string statestr, out string newstatestr, out bool killstatus)
		{
			...
			CheckArgList(arglist, 2, m_killed, out string typestr, out int targetcount, out bool checkprop, out string status_str);
		...
		public static void CheckKilled(Mobile m_killed, Mobile m_killer, Mobile member)
		{
			...
			for (int i = 0; i < mobitems.Count; i++)
			{
				...
				if (mobitems[i] is IXmlQuest quest)
				{
		...
		public static void EventSink_OnKilledBy(Mobile m, Mobile k)
		{
			RegisterKill(m, k);
		}

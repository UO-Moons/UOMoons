...
namespace Server.Gumps
{
				...
				default:
					{
						...
						{    // accept
							...
							else if (m_Item is XmlQuestBook book)
							{
								book.Invalidate();
							}
							else
	...
	public class XmlSimpleGump : Gump
	{
		...
		private readonly List<GumpSelection> gumpSelections = new();
		...
		void LocalAddHtml(string text, int x, int y, int width, int height, int color, bool background, bool scrollbar)
		{
			...
			if (text.StartsWith("#"))
			{
				if (int.TryParse(text.AsSpan(1, text.Length - 1), out int cliloc))
		...
		public XmlSimpleGump(object invoker, string gumptext, string gumptitle, int gumptype, BaseXmlSpawner.KeywordTag tag, Mobile from, XmlGumpCallback gumpcallback) : base(0, 0)
		{
			...
			if (gumptype == 4)
			{
				...
				height += nselections * 40;
			}
			if (gumptype == 5)
			{
				maintext = ParseGumpText(gumptext);
				_ = gumpSelections.Count;
				// the maintext in this case is a width,height specifier so parse it
				string[] args = maintext.Split(',');
				if (args != null && args.Length > 1)
				{
					_ = int.TryParse(args[0].Trim(), out width);
					_ = int.TryParse(args[1].Trim(), out height);
				}
			}
			...
			{ // multiple selection type gump
			  // parse the gump text to get the selections and responses
				...
				{
					int y = 360 + i * 40;
					AddRadio(101, y, 9721, 9724, i == 0, i); // accept/yes radio
					...
					if (args.Length > 1)
					{
						...
						if (args[0].ToLower() == "button")
						{
							...
							if (args.Length > 3)
							{
								...
								{
									int.TryParse(args[1].AsSpan(2), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out gumpid);
								}
								else
								{
									_ = int.TryParse(args[1], out gumpid);
								}
								_ = int.TryParse(args[2], out gx);
								_ = int.TryParse(args[3], out gy);
							...
							if (args[0].ToLower() == "label")
						{
							...
							if (args.Length > 3)
							{
								_ = int.TryParse(args[1], out gx);
								_ = int.TryParse(args[2], out gy);
							...
							if (args.Length > 4)
							{
								_ = int.TryParse(args[4], out color);
							}
						...
						else if (args[0].ToLower() == "html")
						{
							...
							if (args.Length > 5)
							{
								_ = int.TryParse(args[1].Trim(), out gx);
								_ = int.TryParse(args[2].Trim(), out gy);
								_ = int.TryParse(args[3].Trim(), out gwidth);
								_ = int.TryParse(args[4].Trim(), out gheight);
								if (args.Length > 6 && args[5].StartsWith("0x") && args[5].Trim().Length == 8)
								{
									if (!int.TryParse(args[5].Trim().AsSpan(2), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out color))
										color = 0xEFEF5A;
									label = args[6];
								}
								else
						...
						else if (args[0].ToLower() == "textentry")
						{
							...
							if (args.Length > 6)
							{
								_ = int.TryParse(args[6], out color);
							}
						...
						else if (args[0].ToLower() == "radio")
						{
							...
							if (args.Length > 4)
							{
								_ = int.TryParse(args[1].Trim(), out gumpid1);
								_ = int.TryParse(args[2].Trim(), out gumpid2);
								_ = int.TryParse(args[3].Trim(), out gx);
								_ = int.TryParse(args[4].Trim(), out gy);
							}

							bool initial = false;
							if (args.Length > 5)
							{
								_ = bool.TryParse(args[5], out initial);
							}
						...
						else if (args[0].ToLower() == "imagetiled")
						{
							...
							if (args.Length > 5)
							{
								...
								_ = int.TryParse(args[2], out gx);
								_ = int.TryParse(args[3], out gy);
								_ = int.TryParse(args[4], out gwidth);
								_ = int.TryParse(args[5], out gheight);

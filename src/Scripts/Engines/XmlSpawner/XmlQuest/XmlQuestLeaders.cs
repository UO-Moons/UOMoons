...
namespace Server.Engines.XmlSpawner2
{
	public class XmlQuestLeaders
	{
		...
		private static List<QuestRankEntry> QuestRankList = new();
		...
		public static void UpdateQuestRanking(Mobile m, XmlQuestPoints attachment)
		{
			if (QuestRankList == null)
				QuestRankList = new List<QuestRankEntry>();
		...
		public static void WriteQuestLeaderboardXml(string filename, int nranks)
		{
			...
			StreamWriter sw = new(dirname);

			XmlTextWriter xf = new(sw);
			...
			for (int i = 0; i < QuestRankList.Count; i++)
			{
				...
				if (r.Quester != null && !r.Quester.Deleted && r.Rank > 0 && a != null && !a.Deleted)
				{
					...
					if (r.Quester is PlayerMobile mobile && mobile.FactionPlayerState != null)
						factionname = mobile.FactionPlayerState.Faction.ToString();
#endif
					...
					if (r.Quester != null && !r.Quester.Deleted && r.Rank > 0 && a != null && !a.Deleted)
					{
						...
						if (m_attachment != null && m_attachment.nameFilter != null && m_attachment.nameFilter.Length > 0)
						{
							...
							if (args != null)
							{
								...
								foreach (string arg in args)
								{
									if (arg != null && r.Quester.Name != null && (r.Quester.Name.ToLower().Contains(arg.Trim().ToLower(), StringComparison.CurrentCulture)))
									{
										found = true;
										break;
									}

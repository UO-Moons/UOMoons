				...
				default:
					{
						if (info.ButtonID >= 2000)
						{
							...
							{
								if (m_List[index] is IXmlQuest o && !o.Deleted)
								{
			...
			if (list == null)
			{
				...
				if (from.Backpack != null)
				{
					...
					if (packquestitems != null)
					{
						...
						{
							if (packquestitems[i] != null && !packquestitems[i].Deleted && packquestitems[i].Parent is not XmlQuestBook)
			...
			for (int i = index; i < (index + count) && i >= 0 && i < list.Count; ++i)
			{
				...
				if (obj is IXmlQuest e)
				{
					...
					if (e.IsValid)
					{
						...
						if (e.ExpirationString.Contains("Expires in", StringComparison.CurrentCulture))
						{
							substring = e.ExpirationString[11..];
						}
				...
				else if (obj is XmlQuestPoints.QuestEntry e1)
				{
					int y = 96 + (tableIndex++ * 32);
					int color = 67;

					AddLabel(100 - xoffset, y, color, e1.Name);
					...
					AddLabel(515 - xoffset, y, color, e1.Difficulty.ToString());
					...
					XmlQuestAttachment qa = (XmlQuestAttachment)XmlAttach.FindAttachment(from, typeof(XmlQuestAttachment), e1.Name);
					if (qa != null)
					{
					...
					AddLabel(741 - xoffset, y, color, e1.TimesCompleted.ToString());
				}
			}
		}
	}

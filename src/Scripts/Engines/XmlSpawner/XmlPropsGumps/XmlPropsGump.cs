...
namespace Server.Gumps
{
	public class XmlPropertiesGump : Gump
	{
		...
		private void Initialize(int page)
		{
			...
			_ = TotalWidth - PrevWidth - NextWidth - (OffsetSize * 4) - (OldStyle ? SetWidth + OffsetSize : 0);
			...
			for (int i = 0, index = page * EntryCount; i <= count && index < m_List.Count; ++i, ++index)
			{
                ...
                */
				if (o is PropertyInfo info)
				{
					...
					XmlSpawnerDefaults.DefaultEntry de = new();
		...
		private static readonly string[] m_BoolNames = new string[] { "True", "False" };
		private static readonly object[] m_BoolValues = new object[] { true, false };

		private static readonly string[] m_PoisonNames = new string[] { "None", "Lesser", "Regular", "Greater", "Deadly", "Lethal" };
		private static readonly object[] m_PoisonValues = new object[] { null, Poison.Lesser, Poison.Regular, Poison.Greater, Poison.Deadly, Poison.Lethal };
		...
		private class GroupComparer : IComparer
		{
			...
			public int Compare(object x, object y)
			{
				...
				if (x is not DictionaryEntry || y is not DictionaryEntry)
					throw new ArgumentException();

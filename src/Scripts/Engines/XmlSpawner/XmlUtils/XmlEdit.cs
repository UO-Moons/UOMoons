	...
	public class XmlEditDialogGump : Gump
	{
		...
		private class EditDialogTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				if (XmlAttach.FindAttachment(targeted, typeof(XmlDialog)) is not XmlDialog xd)
				{
		...
		{
			return s == null || s.Length < MaxLabelLength ? s : s[..MaxLabelLength];
		}
			...
			bool selectall, bool[] selectionlist, int X, int Y) : base(X, Y)
		{
			...
			else if (m_Dialog.AttachedTo is Mobile mobile)
			{
				Name = mobile.Name;
			}
		...
		private class ListSorter : IComparer
		{
			...
			{
				int xn = ((XmlDialog.SpeechEntry)x).EntryNumber;
				int yn = ((XmlDialog.SpeechEntry)y).EntryNumber;
		...
		{
			XmlEditDialogGump g = new(m_From, false, m_Dialog, Selected,
				...
				default:
					{
						...
						else if (info.ButtonID >= 5000 && info.ButtonID < 5100)
						{
							...
							XmlTextEntryBook book = new(0, string.Empty, m_Dialog.Name, 20, true, new XmlTextEntryBookCallback(ProcessXmlEditBookEntry), args);
		...
		public class XmlConfirmDeleteGump : Gump
		{
			...
			private readonly XmlEditDialogGump m_Gump;

...
namespace Server.Mobiles
{
	public class XmlSpawnerDefaults
	{
		...
		private static ArrayList DefaultEntryList;
	...
	public class XmlAddGump : Gump
	{
		...
		private void DoSaveDefs(Mobile from, string filename)
		{
			...
			// Create the data set
			DataSet ds = new(DefsDataSetName);
		...
		{
			private readonly XmlSpawnerDefaults.DefaultEntry defs;
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				XmlSpawner spawner = new(SpawnId, from.Location.X, from.Location.Y, 0, 0, sname, maxcount,
				...
				case 115: // SaveDefs
					{
						...
						{
							filename = $"{defs.AccountName}-{defs.PlayerName}-{defs.DefsExt}";
						}
						else
						{
							filename = $"{defs.AccountName}-{defs.PlayerName}";
						}
				...
				default:
					{
						...
						if (info.ButtonID >= 5000 && info.ButtonID < 5000 + MaxEntries)
						{
                            ...
                            */
							...
							XmlAddGump newg = new(state.Mobile, defs.StartingLoc, defs.StartingMap, false, defs.ShowExtension, 0, 0);

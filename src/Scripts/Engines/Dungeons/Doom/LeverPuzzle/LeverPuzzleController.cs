...
public class LeverPuzzleController : BaseItem
{
	...
	private ushort MyKey { get; set; }

	[CommandProperty(AccessLevel.GameMaster)]
	private ushort TheirKey { get; set; }
	...
	private static Item AddLeverPuzzlePart(int[] loc, Item newitem)
	{
	...
	public override void OnAfterDelete()
	{
		...
		if (_mBox is { Deleted: false })
		{
			_mBox.Delete();
		}
	}

	private static void NukeItemList(IReadOnlyCollection<Item> list)
	{
		if (list == null || list.Count == 0) return;
		foreach (var item in list.Where(item => item is { Deleted: false }))
		{
			item.Delete();
		}
	}
	...
	public virtual LeverPuzzleLever GetLever(int index)
	{
		...
		return lever is { Deleted: false } ? lever : null;
	}
	...
	public virtual void LeverPulled(ushort code)
	{
		...
		{
			_lTimer = Timer.DelayCall(TimeSpan.FromSeconds(30.0), ResetPuzzle);
			return;
		}
	...
	private class LampRoomTimer : Timer
	{
		...
		{
			m_Ticks++;
			List<Mobile> mobiles = m_Controller._mLampRoom.GetMobiles();

			if (m_Ticks >= 71 || m_Controller._mLampRoom.GetPlayerCount() == 0)
			{
				foreach (var mobile in mobiles.Where(mobile => mobile is { Deleted: false, IsDeadBondedPet: false }))
				{
					mobile.Kill();
				}
			...
			else
			{
				...
				foreach (var mobile in mobiles.Where(IsValidDamagable))
				{
					...
					if (Utility.Random((int)(m_Level & ~0xfffffffc), 3) == 3)
					{
						mobile.ApplyPoison(mobile, Pa2[m_Level]);
					}
					if (m_Ticks % 12 == 0 && m_Level > 0 && mobile.Player)
					{
						mobile.SendLocalizedMessage(Pa[m_Level][0], Pa[m_Level][1]);
					}
				}
				for (var i = 0; i <= m_Level; i++)
				{
					SendLocationEffect(RandomPointIn(LrRect, -1), 0x36B0, Utility.Random(150, 200), 0, Pa[m_Level][2]);
				}
			}
		}
	}
	...
	private static void MoveMobileOut(Mobile m)
	{
		switch (m)
		{
			case null:
				return;
			case PlayerMobile when !m.Alive:
			{
	...
	private static bool AniSafe(Mobile m)
	{
		return m != null && !TransformationSpellHelper.UnderTransformation(m) && m.BodyMod == 0 && m.Alive;
	}

	private static IEntity ZAdjustedIeFromMobile(IEntity m, int zDelta)
	{
		return new Entity(Serial.Zero, new Point3D(m.X, m.Y, m.Z + zDelta), m.Map);
	}

	private static void DoDamage(IDamageable m, int min, int max, bool poison)
	{
	...
	private static Point3D RandomPointIn(Point3D point, int range)
	{
		return RandomPointIn(point.X - range, point.Y - range, range * 2, range * 2, point.Z);
	}

	private static Point3D RandomPointIn(Rectangle2D rect, int z)
	{
		return RandomPointIn(rect.X, rect.Y, rect.Height, rect.Width, z);
	}

	private static Point3D RandomPointIn(int x, int y, int x2, int y2, int z)
	{
		return new Point3D(Utility.Random(x, x2), Utility.Random(y, y2), z);
	}

	private static void PlaySounds(Point3D location, int[] sounds)
	{
	...
	private static void PlayerSendAscii(Mobile player, int index)
	{
	...
	private static void PohMessage(Mobile from, int index)
	{
	...
	public static readonly Point3D LpCenter = new(324, 64, -1);

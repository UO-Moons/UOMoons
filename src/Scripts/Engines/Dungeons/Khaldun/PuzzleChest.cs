...
namespace Server.Items
{
	...
	public class PuzzleChestSolution
	{
		...
		public PuzzleChestCylinder First { get => Cylinders[0];
			private init => Cylinders[0] = value; }
		public PuzzleChestCylinder Second { get => Cylinders[1];
			private init => Cylinders[1] = value; }
		public PuzzleChestCylinder Third { get => Cylinders[2];
			private init => Cylinders[2] = value; }
		public PuzzleChestCylinder Fourth { get => Cylinders[3];
			private init => Cylinders[3] = value; }
		public PuzzleChestCylinder Fifth { get => Cylinders[4];
			private init => Cylinders[4] = value; }

		private static PuzzleChestCylinder RandomCylinder()
		{
	...
	{
		private const int HintsCount = 3;
		private readonly TimeSpan m_CleanupTime = TimeSpan.FromHours(1.0);
		...
		private PuzzleChestCylinder[] Hints { get; set; } = new PuzzleChestCylinder[HintsCount];

		private PuzzleChestCylinder FirstHint { get => Hints[0]; set => Hints[0] = value; }
		private PuzzleChestCylinder SecondHint { get => Hints[1]; set => Hints[1] = value; }
		private PuzzleChestCylinder ThirdHint { get => Hints[2]; set => Hints[2] = value; }
		...
		public override bool CheckLocked(Mobile from)
		{
			if (Locked)
			{
				...
				solution = solution != null ? new PuzzleChestSolution(solution) : new PuzzleChestSolution(PuzzleChestCylinder.None, PuzzleChestCylinder.None, PuzzleChestCylinder.None, PuzzleChestCylinder.None, PuzzleChestCylinder.None);
		...
		private void SubmitSolution(Mobile m, PuzzleChestSolution solution)
		{
		...
		{
			LootHelpers.PuzzleChestLoot(this);
			Solution = new PuzzleChestSolution();
		}

		private void CleanupGuesses()
		{
			List<Mobile> toDelete = (from kvp in m_Guesses where DateTime.UtcNow - kvp.Value.When > m_CleanupTime select kvp.Key).ToList();

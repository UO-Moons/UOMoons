...
namespace Server.Items
{
    public class NavreysController : Item
    {
        ...
        {
	        return World.Items.Values.Any(item => item is NavreysController && !item.Deleted);
        }
        ...
        private bool AllPillarsHot
        {
            get
            {
	            return m_Pillars.All(pillar => pillar is { Deleted: false, State: NavreysPillarState.Hot });
            }
        }

        private NavreysController()
            : base(0x1F13)
        {
        ...
        {
            if (m_Navrey is { Deleted: false, Alive: true })
                SetAllPillars(NavreysPillarState.On);
        }
        ...
        private class RockRainTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                ...
                Point3D orig = new(dest.X - Utility.RandomMinMax(3, 4), dest.Y - Utility.RandomMinMax(8, 9), dest.Z + Utility.RandomMinMax(41, 43));
                ...
                foreach (Mobile m in eable)
                {
                    ...
                    {
                        m.Damage(amount);
                        m.SendVisibleDamageEveryone(amount);
                    }
                    else
                    {
                        m.RevealingAction();

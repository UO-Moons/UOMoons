...
namespace Server.Items
{
    public class UnderworldPuzzleItem : BaseDecayingItem
    {
        ...
        public bool SubmitSolution(Mobile m, UnderworldPuzzleSolution solution)
        {
            ...
            if (solution.Matches(Solution))
            {
                ...
                if (item != null)
                {
	                item.Amount = item switch
	                {
		                VoidEssence or SilverSerpentVenom or ToxicVenomSac => 30,
		                LuckyCoin => Utility.RandomMinMax(2, 6),
		                _ => item.Amount
	                };
    ...
    public class UnderworldPuzzleSolution
    {
        ...
        public PuzzlePiece First { get => Rows[0];
	        private set => Rows[0] = value;
        }
        public PuzzlePiece Second { get => Rows[1];
	        private set => Rows[1] = value;
        }
        public PuzzlePiece Third { get => Rows[2];
	        private set => Rows[2] = value;
        }
        public PuzzlePiece Fourth { get => Rows[3];
	        private set => Rows[3] = value;
        }
        ...
        {
	        return Rows.Where((t, i) => t == check.Rows[i]).Count();
        }
        ...
        private void LoadStartSolution(int index)
        {
            Index = index;
    ...
    public class UnderworldPuzzleGump : Gump
    {
        ...
        private UnderworldPuzzleGump(Mobile from, UnderworldPuzzleItem item, int row)
            : base(5, 30)
        {
            if (row > 3) row = 3;
            if (row < 0) row = 0;
        ...
        private static int GetTotalPieces(PuzzlePiece piece)
        {
	        return piece switch
	        {
		        PuzzlePiece.RedSingle => 1,
		        PuzzlePiece.BlueSingle => 1,
		        PuzzlePiece.GreenSingle => 1,
		        PuzzlePiece.RedDouble => 2,
		        PuzzlePiece.BlueDouble => 2,
		        PuzzlePiece.GreenDouble => 2,
		        PuzzlePiece.RedTriple => 3,
		        PuzzlePiece.BlueTriple => 3,
		        PuzzlePiece.GreenTriple => 3,
		        PuzzlePiece.RedBar => 4,
		        PuzzlePiece.BlueBar => 4,
		        PuzzlePiece.GreenBar => 4,
		        _ => 0
	        };
        }
        ...
        private static void SplitPiecesDown(ref PuzzlePiece movingPiece, ref PuzzlePiece movingToPiece)
        {
            ...
            {
	            t = movingAmount switch
	            {
		            //greater = movingPiece;
		            //t = (int)movingToPiece + (movingAmount - moveToAmount);
		            //t -= 1;
		            4 when moveToAmount == 3 => (int)movingToPiece - 2,
		            4 when moveToAmount == 2 => (int)movingToPiece,
		            4 when moveToAmount == 1 => (int)movingToPiece + 2,
		            2 when moveToAmount == 1 => (int)movingToPiece,
		            3 when moveToAmount == 2 => (int)movingToPiece - 1,
		            3 when moveToAmount == 1 => (int)movingToPiece + 1,
		            _ => t
	            };
            }
            else
        ...
        private static PuzzleColor GetColor(PuzzlePiece piece)
        {
	        return piece switch
	        {
		        PuzzlePiece.RedSingle => PuzzleColor.Red,
		        PuzzlePiece.RedDouble => PuzzleColor.Red,
		        PuzzlePiece.RedTriple => PuzzleColor.Red,
		        PuzzlePiece.RedBar => PuzzleColor.Red,
		        PuzzlePiece.BlueSingle => PuzzleColor.Blue,
		        PuzzlePiece.BlueDouble => PuzzleColor.Blue,
		        PuzzlePiece.BlueTriple => PuzzleColor.Blue,
		        PuzzlePiece.BlueBar => PuzzleColor.Blue,
		        PuzzlePiece.GreenSingle => PuzzleColor.Green,
		        PuzzlePiece.GreenDouble => PuzzleColor.Green,
		        PuzzlePiece.GreenTriple => PuzzleColor.Green,
		        PuzzlePiece.GreenBar => PuzzleColor.Green,
		        _ => PuzzleColor.Red
	        };
        }

        private static int GetPuzzlePieceId(PuzzlePiece piece)
        {
	        return piece switch
	        {
		        PuzzlePiece.None => 0x0,
		        PuzzlePiece.RedSingle => 0x2A62,
		        PuzzlePiece.RedDouble => 0x2A62,
		        PuzzlePiece.RedTriple => 0x2A62,
		        PuzzlePiece.BlueSingle => 0x2A3A,
		        PuzzlePiece.BlueDouble => 0x2A3A,
		        PuzzlePiece.BlueTriple => 0x2A3A,
		        PuzzlePiece.GreenSingle => 0x2A4E,
		        PuzzlePiece.GreenDouble => 0x2A4E,
		        PuzzlePiece.GreenTriple => 0x2A4E,

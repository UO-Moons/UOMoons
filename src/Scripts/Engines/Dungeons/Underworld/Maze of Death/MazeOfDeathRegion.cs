...
{
	private static readonly Point2D[] m_Points =
	{
	...
	public static void Initialize()
	{
		...
		while (toAdd > 0)
		{
			int x = Utility.RandomMinMax(_trapCorridor.X, _trapCorridor.X + _trapCorridor.Width);
			int y = Utility.RandomMinMax(_trapCorridor.Y, _trapCorridor.Y + _trapCorridor.Height);
	...
	private static readonly Rectangle2D m_FrontEntrance = new(1057, 1062, 8, 6);
	private static readonly Rectangle2D m_Room = new(1065, 1055, 11, 11);
	private static readonly Rectangle2D m_Corridor = new(1056, 990, 9, 72);        
	private static readonly Rectangle2D m_PuzzleRoom = new(1065, 1023, 8, 8);
	private static readonly Rectangle2D m_RearEntrance = new(1056, 986, 9, 4);
	...
	private static Rectangle2D _deathBounds = new(1057, 1062, 7, 5);
	private static Rectangle2D _trapCorridor = new(1056, 991, 9, 70);

	public static List<Point2D> Path { get; private set; }

	private MazeOfDeathRegion()
	...
	public override bool OnBeginSpellCast(Mobile m, ISpell s)
	{
		...
		if (s is not Spells.Sixth.MarkSpell && s is not Spells.Seventh.GateTravelSpell &&
		    s is not Spells.Third.TeleportSpell)
	...
	{
		if (m.AccessLevel >= AccessLevel.Counselor || t is not Spells.Third.TeleportSpell.InternalTarget)
	...
	{
		if (m is not PlayerMobile { Alive: true } pm || (!m_FrontEntrance.Contains(pm.Location) && !m_RearEntrance.Contains(pm.Location)))
			return;
		...
		{
			pm.LocalOverheadMessage(MessageType.Regular, 946,
				pm.Backpack?.FindItemByType<GoldenCompass>(false) != null ? 1113582 : 1113581);
	...
	public override void OnLocationChanged(Mobile m, Point3D oldLocation)
	{
		...
		if (m.Alive)
		{
			if (_trapCorridor.Contains(m.Location) && !Path.Contains(new Point2D(m.Location.X, m.Location.Y)))
			{
				SpringTrap(m);
			}
		...
		else if (_trapCorridor.Contains(m.Location))
		{
	...
	public override void OnDeath(Mobile m)
	{
		...
		if (m.Player && _trapCorridor.Contains(m.Location))
	...
	private void Kick_Callback(object o)
	{
		Mobile m = (Mobile)o;
	...
	private static void KickToEntrance(Mobile from)
	{
		...
		int x = Utility.RandomMinMax(_deathBounds.X, _deathBounds.X + _deathBounds.Width);
		int y = Utility.RandomMinMax(_deathBounds.Y, _deathBounds.Y + _deathBounds.Height);

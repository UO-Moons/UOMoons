...
public class PuzzleBox : Item
{
	...
	private PuzzleType PuzzleType { get; set; }
	...
	public override void OnDoubleClick(Mobile from)
	{
		...
		else if (from.Backpack != null)
		{
			...
			needed = PuzzleType switch
			{
				PuzzleType.WestBox => typeof(MagicKey),
				PuzzleType.EastBox => typeof(MagicKey),
				PuzzleType.NorthBox => typeof(GoldPuzzleKey),
				_ => typeof(MagicKey)
			};
			...
			{
				if (PuzzleType == PuzzleType.NorthBox)
			...
			if (puzzle != null)
			{
				if (!from.Backpack.TryDropItem(from, puzzle, true))
					...
					from.SendMessage("You received a puzzle board.");
			}
			else
	...
	public override void Deserialize(GenericReader reader)
	{
		...
		reader.ReadInt();
		PuzzleType = (PuzzleType)reader.ReadInt();
	}
}

...
public class ExodusSummoningAlter : BaseDecayingItem
{
	...
	private class SummoningTarget : Target
	{
		...
		{
			return itemId is >= 0x149F and <= 0x14D6;
		}
		...
		protected override void OnTarget(Mobile from, object targeted)
		{
			if (targeted is StaticTarget targ)
			{
				...
				{
					bool alter = from.Map.GetItemsInRange(targ.Location, 5).Any(x => x is ExodusTomeAltar);
	...
	{
		return ClockworkExodus.Instances.FirstOrDefault(m => m.Region.IsPartOf("Ver Lor Reg") && ((m.Hits >= m.HitsMax * 0.60 && ClockworkExodus.MinHits >= m.HitsMax * 0.60) || (m.Hits >= m.HitsMax * 0.75))) != null;
	}
}

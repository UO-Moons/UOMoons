...
public class ExodusNexus : NexusAddon
{
	...
	private readonly Dictionary<Mobile, DamageTimer> m_DamageTable = new();
	...
	[Constructable]
	private ExodusNexus(int sideLength)
	{
		Hue = 1987;
	...
	private void DoDamage(Mobile to)
	{
		...
		if (m_DamageTable.ContainsKey(to))
			return;
	...
	private void Solve(Mobile from)
	{
	...
	private class DamageTimer : Timer
	{
		...
		private void End()
		{
			m_Nexus.m_DamageTable.Remove(m_To);
			m_To.Frozen = false;
	...
	public override void Deserialize(GenericReader reader)
	{
		...
		Path = new Node[reader.ReadInt()];

		for (int i = 0; i < Path.Length; i++)
		{
			Path[i] = new Node(reader.ReadInt(), reader.ReadInt());
		}
	...
	private class NexusGameGump : Gump
	{
		private readonly ExodusNexus m_Nexus;

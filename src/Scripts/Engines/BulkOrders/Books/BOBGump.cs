...
public class BOBGump : Gump
{
	...
	public static Item Reconstruct(object obj)
	{
		Item item = null;

		if (obj is BOBLargeEntry entry)
			item = entry.Reconstruct();
		else if (obj is BOBSmallEntry entry1)
			item = entry1.Reconstruct();
	...
	{
		if (obj is BOBLargeEntry le)
		{
			return CheckFilter(le.Material, le.AmountMax, true, le.RequireExceptional, le.DeedType, le.Entries.Length > 0 ? le.Entries[0].ItemType : null);
		}
		else if (obj is BOBSmallEntry se)
		{
			return CheckFilter(se.Material, se.AmountMax, false, se.RequireExceptional, se.DeedType, se.ItemType);
		}
	...
	public bool CheckFilter(BulkMaterialType mat, int amountMax, bool isLarge, bool reqExc, BODType deedType, Type itemType)
	{
		...
		switch (f.Quantity)
		{
			case 1 when amountMax != 10:
				return false;
			case 2 when amountMax != 15:
				return false;
			case 3 when amountMax != 20:
				return false;
		}
	...
	public static object GetMaterialName(BulkMaterialType mat, BODType type, Type itemType)
	{
		switch (type)
		{
			case BODType.Smith:
			...
			case BODType.Tailor:
				{
					switch (mat)
					{
						case BulkMaterialType.None:
							{
								return itemType.IsSubclassOf(typeof(BaseArmor)) || itemType.IsSubclassOf(typeof(BaseShoes)) ? 1062235 : (object)1044286;
							}
			...
			default:
				{
					...
					_ = canDrop || (m_Book.RootParent is PlayerVendor);
		...
		public override void OnResponse(Mobile from, string text)
		{
			...
			else if (m_Object == null)
			{
				...
				if (from is PlayerMobile mobile)
					from.SendGump(new BOBGump(mobile, m_Book, m_Page, m_List));
			}
			...
			else if (m_Object is BOBSmallEntry entry1)
			{
				entry1.Price = price;
		...
		for (int i = index; i < (index + count) && i >= 0 && i < list.Count; ++i)
		{
			...
			if (obj is BOBLargeEntry le)
			{
				...
				for (int j = 0; j < le.Entries.Length; ++j)
				{
					...
					object name = GetMaterialName(le.Material, le.DeedType, sub.ItemType);

					if (name is int @int)
						AddHtmlLocalized(316, y, 100, 20, @int, LabelColor, false, false);
					else if (name is string @string)
						AddLabel(316, y, 1152, @string);

					AddLabel(421, y, 1152, string.Format("{0} / {1}", sub.AmountCur, le.AmountMax));
			...
			else if (obj is BOBSmallEntry se)
			{
				...
				if (canDrop || (canBuy && se.Price > 0))
				{
					AddButton(579, y + 2, 2117, 2118, 6 + (i * 2), GumpButtonType.Reply, 0);
					AddLabel(495, y, 1152, se.Price.ToString());
				}

				AddHtmlLocalized(61, y, 50, 32, 1062224, LabelColor, false, false); // Small

				AddHtmlLocalized(103, y, 130, 32, se.Number, LabelColor, false, false);

				if (se.RequireExceptional)
					AddHtmlLocalized(235, y, 80, 20, 1060636, LabelColor, false, false); // exceptional
				else
					AddHtmlLocalized(235, y, 80, 20, 1011542, LabelColor, false, false); // normal

				object name = GetMaterialName(se.Material, se.DeedType, se.ItemType);
				...
				AddLabel(421, y, 1152, string.Format("{0} / {1}", se.AmountCur, se.AmountMax));
			}
		}
	}
}

	...
	public abstract class SmallBOD : BaseItem
	{
		...
		public void EndCombine(Mobile from, object o)
		{
			if (o is Item item && item.IsChildOf(from.Backpack))
			{
				...
				else if (Type == null || (objectType != Type && !objectType.IsSubclassOf(Type)) || (o is not BaseWeapon && o is not BaseArmor && o is not BaseClothing))
				{
				...
				else
				{
					...
					if (o is BaseArmor armor)
						material = GetMaterial(armor.Resource);
					else if (o is BaseClothing clothing)
						material = GetMaterial(clothing.Resource);

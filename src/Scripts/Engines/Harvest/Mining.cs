...
{
	private static Mining _mSystem;

	public static Mining System => _mSystem ??= new Mining();
	...
	private Mining()
	{
		...
		oreAndStone.Tiles = MMountainAndCaveTiles;
	...
	{
		if (def != OreAndStone) return base.GetResourceType(from, tool, def, map, loc, resource);
		if (from is PlayerMobile {StoneMining: true, ToggleMiningStone: true} && from.Skills[SkillName.Mining].Base >= 100.0 && 0.1 > Utility.RandomDouble())
				...
				try
				{
					...
					if (Activator.CreateInstance(res.Types[2], 25) is BaseCreature spawned)
					{
						...
						for (int i = 0; i < Helpers.Offsets.Length; i += 2)
						{
							...
							if (Math.Abs(z - from.Z) >= 10 || !map.CanSpawnMobile(x, y, z)) continue;
	...
	{
		// You can't mine there.// You can't mine that.
		from.SendLocalizedMessage(toHarvest is LandTarget ? 501862 : 501863);
	}

	#region Tile lists
	private static readonly int[] MMountainAndCaveTiles = {
	...
	private static readonly int[] MSandTiles = {

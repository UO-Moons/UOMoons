...
{
	private static readonly int[] MSatchelHues = {0x1C, 0x37, 0x71, 0x3A, 0x62, 0x44, 0x59, 0x13, 0x21, 0x3, 0xD, 0x3F,}; // TODO update

	private static readonly int[] MRewardBagHues = {
		//	from,	to,
	...
	{
		return MSatchelHues[Utility.Random(MSatchelHues.Length)];
	}
	...
	public static int RewardBagHue()
	{
		...
		int row = Utility.Random(MRewardBagHues.Length / 2) * 2;

		return Utility.RandomMinMax(MRewardBagHues[row], MRewardBagHues[row + 1]);
	}
	...
	{
		var recipes = GetRecipe(Core.TOL ? new[] { 400, 401, 402 } : new[] { 400, 401, 402, 454 });
	...
	{
		return 0.005 >= Utility.RandomDouble() ? Loot.Construct(ElvishFurniture) : null;
	}

	private static readonly Type[] ElvishFurniture =
	{
	...
	public static RecipeScroll GetRecipe(Array list)
	{
		...
		switch (mid)
		{
			// only lesser recipes in list
			case -1 when ran <= 0.33:
				return new RecipeScroll(recipes[Utility.Random(list.Length)]);
			// only greater recipes in list
			case 0 when ran <= 0.01:
				return new RecipeScroll(recipes[Utility.Random(list.Length)]);
			default:
			{
				switch (ran)

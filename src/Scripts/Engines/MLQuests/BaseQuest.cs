...
namespace Server.Engines.Quests
{
	public class BaseQuest
	{
		...
		public BaseQuestItem StartingItem => m_Quester is BaseQuestItem item ? item : null;
		public MondainQuester StartingMobile => m_Quester is MondainQuester quester ? quester : null;
		...
		public virtual void OnResign(bool resignChain)
		{
			...
			for (int i = 0; i < Objectives.Count; i++)
			{
				if (Objectives[i] is ObtainObjective obtain)
				{
					_ = QuestHelper.RemoveStatus(Owner, obtain.Obtain);
				}
				else if (Objectives[i] is DeliverObjective deliver)
				{

...
public class HumilityShrinePersistence : Item
{
	...
	*/
	...
	private Rectangle2D _mRec = new(4273, 3696, 2, 2);

	public override bool HandlesOnMovement => true;

	public override void OnMovement(Mobile m, Point3D oldLocation)
	{
		if (m is not PlayerMobile || !_mRec.Contains(m) || m.Backpack == null) return;
		Item item = m.Backpack.FindItemByType(typeof(GreyCloak)) ?? m.FindItemOnLayer(Layer.Cloak);

		if (item is not GreyCloak) return;

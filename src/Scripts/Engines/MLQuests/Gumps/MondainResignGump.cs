...
public class MondainResignGump : Gump
{
	...
	{
		if (info.ButtonID != (int)Buttons.Okay || _mQuest == null)
			return;

		if (_mQuest.ChainId != QuestChain.None && info.IsSwitched((int)Radios.Chain))
		{
			_mQuest.OnResign(true);
		}
		...
		if (info.IsSwitched((int)Radios.None) && _mQuest.Owner != null)
			_mQuest.Owner.SendGump(new MondainQuestGump(_mQuest.Owner));
	}
}

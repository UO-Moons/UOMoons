...
public class Titles
{
	...
	public static void AwardKarma(Mobile m, int offset, bool message)
	{
		switch (offset)
		{
			case > 0 when m is PlayerMobile {KarmaLocked: true}:
			case > 0 when m.Karma >= MaxKarma:
				return;
			case > 0:
			{
				offset -= m.Karma / 100;
	...
	public static List<string> GetFameKarmaEntries(Mobile m)
	{
		...
		for (var i = 0; i < MFameEntries.Length; ++i)
		{
			...
			list.AddRange(from ke in karmaEntries let title = new StringBuilder() where (karma >= 0 && ke.Karma >= 0 && karma >= ke.Karma) || (karma < 0 && ke.Karma < 0 && karma < ke.Karma) select title.AppendFormat(ke.Title, m.Name, m.Female ? "Lady" : "Lord").ToString());
		}

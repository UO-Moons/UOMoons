...
{
	private readonly bool _description;
	...
	protected void AddGumpLayout()
	{
		...
		AddCallbackButton(445, 315, 4005, 4007, 0, GumpButtonType.Reply, 0, _ => { });

		AddHtmlLocalized(55, 70, 160, 16, 1115026, 0xFFFF, false, false); // Paperdoll Name (Prefix)
		AddCallbackButton(20, 70, 4005, 4007, 10001, GumpButtonType.Reply, 0, _ =>
		{
		...
		AddCallbackButton(20, 92, 4005, 4007, 10002, GumpButtonType.Reply, 0, _ =>
		{
		...
		AddCallbackButton(20, 114, 4005, 4007, 10003, GumpButtonType.Reply, 0, _ =>
		{
		...
		AddCallbackButton(20, 136, 4005, 4007, 10004, GumpButtonType.Reply, 0, _ =>
		{
		...
		if (TitleType > TitleType.PaperdollPrefix && HasTitle())
		{
			if (!TitleClearing)
			{
				...
				AddCallbackButton(20, 315, 4005, 4007, 10005, GumpButtonType.Reply, 0, _ =>
				{
					TitleClearing = true;
					Category = TitleCategory.None;
					Refresh();
			...
			else
			{
				...
				AddCallbackButton(445, 275, 4005, 4007, 10006, GumpButtonType.Reply, 0, _ =>
				{
		...
		{
			AddHtmlLocalized(215, 50, 315, 16, ShowingDescription ? 1115025 : 1044011, 0xFFFF, false, false);
		}
	...
	private void BuildPaperdollPrefix()
	{
		...
		AddCallbackButton(20, 190, 4005, 4007, 1, GumpButtonType.Reply, 0, _ =>
		{
		...
		if (Category == TitleCategory.FameKarma)
		{
			if (!ShowingDescription)
			{
				...
				AddCallbackButton(225, 70, 4005, 4007, 2, GumpButtonType.Reply, 0, _ =>
				{
			...
			else
			{
				...
				AddCallbackButton(445, 275, 4005, 4007, 99, GumpButtonType.Reply, 0, _ =>
				{
					if (TitleSelected >= 0 && TitleSelected < fameKarma.Count)
						title = fameKarma[TitleSelected];

					User.FameKarmaTitle = title ?? null;
	...
	private void BuildPaperdollSuffix()
	{
		...
		AddCallbackButton(20, 190, 4005, 4007, 100, GumpButtonType.Reply, 0, _ =>
		{
		...
		ChampionTitleInfo info = User.ChampionTitles;

		if (info != null && info.HasChampionTitle(User))
		{
			AddHtmlLocalized(55, 212, 160, 16, 1115032, 0xFFFF, false, false); // Monster
			AddCallbackButton(20, 212, 4005, 4007, 101, GumpButtonType.Reply, 0, _ =>
			{
		...
		if (Category == TitleCategory.Skills)
		{
			...
			else
			{
				...
				AddHtmlLocalized(480, 275, 80, 16, 1011046, 0xFFFF, false, false); // APPLY
				AddCallbackButton(445, 275, 4005, 4007, 199, GumpButtonType.Reply, 0, _ =>
				{
					AddHtmlLocalized(225, 315, 200, 16, 1115036, 0xFFFF, false, false); // TITLE APPLIED
		...
		else if (Category == TitleCategory.Champion && info != null)
		{
			if (!ShowingDescription || TitleSelected == -1)
			{
				...
				if (info.Harrower > 0)
				{
					AddHtml(260, y, 245, 16, Color("#FFFFFF",
						$": {Titles.HarrowerTitles[Math.Min(Titles.HarrowerTitles.Length, info.Harrower) - 1]} of Evil"), false, false);
					AddCallbackButton(225, y, 4005, 4007, 295, GumpButtonType.Reply, 0, _ =>
					{
						TitleSelected = 295;
						ShowingDescription = true;
						Refresh();
				...
				for (int i = 0; i < ChampionSpawnInfo.Table.Length; i++)
				{
					...
					AddHtml(260, y + index * 22, 245, 16, Color("#FFFFFF",
						$": {champInfo.LevelNames[Math.Min(offset, champInfo.LevelNames.Length) - 1]} of the {champInfo.Name}"), false, false);
			...
			else
			{
				...
				AddHtmlLocalized(480, 275, 80, 16, 1011046, 0xFFFF, false, false); // APPLY
				AddCallbackButton(445, 275, 4005, 4007, 299, GumpButtonType.Reply, 0, _ =>
				{
					AddHtmlLocalized(225, 315, 200, 16, 1115036, 0xFFFF, false, false); // TITLE APPLIED
	...
	private void BuildOverheadName()
	{
		...
		AddCallbackButton(20, y, 4005, 4007, 300, GumpButtonType.Reply, 0, _ =>
		{
			Category = TitleCategory.Skills;
			ShowingDescription = false;
			TitleClearing = false;
		...
		if (rewards is {Count: > 0})
		{
			...
			AddCallbackButton(20, y, 4005, 4007, 5435, GumpButtonType.Reply, 0, _ =>
			{
		...
		if (g != null)
		{
			...
			AddCallbackButton(20, y, 4005, 4007, 301, GumpButtonType.Reply, 0, _ =>
			{
		...
		if (Category == TitleCategory.Skills)
		{
			...
			else
			{
				...
				//else
				//{
				var title = "the " + User.Skills[(SkillName)TitleSelected].Info.Title;
				var desc = 1115056 + TitleSelected;
				//}
				...
				AddHtmlLocalized(480, 275, 80, 16, 1011046, 0xFFFF, false, false); // APPLY
				AddCallbackButton(445, 275, 4005, 4007, 399, GumpButtonType.Reply, 0, _ =>
				{
					AddHtmlLocalized(225, 315, 200, 16, 1115036, 0xFFFF, false, false); // TITLE APPLIED
		...
		else if (Category == TitleCategory.Guild && g != null)
		{
			if (!ShowingDescription || TitleSelected == -1)
			{
				...
				AddCallbackButton(225, 70, 4005, 4007, 397, GumpButtonType.Reply, 0, _ =>
				{
					ShowingDescription = true;
					TitleSelected = 351;
					Refresh();
			...
			else
			{
				...
				AddHtmlLocalized(480, 275, 80, 16, 1011046, 0xFFFF, false, false); // APPLY
				AddCallbackButton(445, 275, 4005, 4007, 398, GumpButtonType.Reply, 0, _ =>
				{
					AddHtmlLocalized(225, 315, 200, 16, 1115036, 0xFFFF, false, false); // TITLE APPLIED
					Refresh(false);
		...
		else if (Category == TitleCategory.RewardTitles && rewards is {Count: > 0})
		{
			if (!ShowingDescription || TitleSelected == -1)
			{
				int index = 0;
			...
			else if (TitleSelected >= 0 && TitleSelected < rewards.Count)
			{
				...
				AddHtmlLocalized(480, 275, 80, 16, 1011046, 0xFFFF, false, false); // APPLY
				AddCallbackButton(445, 275, 4005, 4007, 5789, GumpButtonType.Reply, 0, _ =>
				{
					AddHtmlLocalized(225, 315, 200, 16, 1115036, 0xFFFF, false, false); // TITLE APPLIED
					Refresh(false);
	...
	private void BuildSubtitle()
	{
		...
		AddHtmlLocalized(55, y, 160, 16, 1115030, 0xFFFF, false, false); // Skills
		AddCallbackButton(20, y, 4005, 4007, 400, GumpButtonType.Reply, 0, _ =>
		{
			Category = TitleCategory.Skills;
			ShowingDescription = false;
			TitleClearing = false;
		...
		{
			AddHtmlLocalized(55, y, 160, 16, 1115033, 0xFFFF, false, false); // GUILD
			AddCallbackButton(20, y, 4005, 4007, 401, GumpButtonType.Reply, 0, _ =>
			{
				Category = TitleCategory.Guild;
				ShowingDescription = false;
				TitleClearing = false;
		...
		if (User.RewardTitles is {Count: > 0})
		{
			AddHtmlLocalized(55, y, 160, 16, 1115034, 0xFFFF, false, false); // Rewards
			AddCallbackButton(20, y, 4005, 4007, 402, GumpButtonType.Reply, 0, _ =>
			{
				Category = TitleCategory.RewardTitles;
				ShowingDescription = false;
				TitleClearing = false;
		...
		if (vetTitles != null && vetTitles.Count > 0)
		{
			...
			AddCallbackButton(20, y, 4005, 4007, 403, GumpButtonType.Reply, 0, _ =>
			{
		...
		if (Category == TitleCategory.Skills)
		{
			...
			else
			{
				...
				AddHtmlLocalized(480, 275, 80, 16, 1011046, 0xFFFF, false, false); // APPLY
				AddCallbackButton(445, 275, 4005, 4007, 102, GumpButtonType.Reply, 0, _ =>
				{
					AddHtmlLocalized(225, 315, 200, 16, 1115036, 0xFFFF, false, false); // TITLE APPLIED
		...
		else if (Category == TitleCategory.Guild && guild != null && User.GuildTitle != null)
		{
			if (!ShowingDescription || TitleSelected == -1)
			{
				AddHtml(260, 70, 245, 16, Color("#FFFFFF",
					$"{Utility.FixHtml(User.GuildTitle)}, {Utility.FixHtml(guild.Name)}"), false, false);
				AddCallbackButton(225, 70, 4005, 4007, 500, GumpButtonType.Reply, 0, _ =>
				{
					TitleSelected = 1;
					ShowingDescription = true;
					Refresh();
			...
			else
			{
				...
				AddHtmlLocalized(225, 220, 160, 16, 1115029, 0xFFFF, false, false); // Subtitle
				AddHtml(275, 240, 245, 16, Color("#FFFFFF",
					$"{Utility.FixHtml(User.GuildTitle)}, {Utility.FixHtml(guild.Name)}"), false, false);

				AddHtmlLocalized(225, 275, 200, 16, 1115035, 0xFFFF, false, false); // Do you wish to apply this title?

				AddHtmlLocalized(480, 275, 80, 16, 1011046, 0xFFFF, false, false); // APPLY
				AddCallbackButton(445, 275, 4005, 4007, 599, GumpButtonType.Reply, 0, _ =>
				{
					AddHtmlLocalized(225, 315, 200, 16, 1115036, 0xFFFF, false, false); // TITLE APPLIED
		...
		else if (Category == TitleCategory.RewardTitles && User.RewardTitles != null && User.RewardTitles.Count > 0)
		{
				...
				for (int i = 0; i < User.RewardTitles.Count; i++)
				{
					...
					switch (title)
					{
						case int title1:
							...
							//}
							//else
							AddHtmlLocalized(260, 70 + index * 22, 245, 16, title1, 0xFFFF, false, false);
							break;
						case string s:
			...
			else if (TitleSelected >= 0 && User.RewardTitles != null && TitleSelected < User.RewardTitles.Count)
			{
				...
				switch (description)
				{
					case int description1:
						AddHtmlLocalized(225, 70, 270, 140, description1, 0xFFFF, false, false);
						break;
					case string s:
						AddHtml(225, 70, 270, 140, Color("#FFFFFF", s), false, false);
						break;
				...
				AddHtmlLocalized(480, 275, 80, 16, 1011046, 0xFFFF, false, false); // APPLY
				AddCallbackButton(445, 275, 4005, 4007, 699, GumpButtonType.Reply, 0, _ =>
				{
					AddHtmlLocalized(225, 315, 200, 16, 1115036, 0xFFFF, false, false); // TITLE APPLIED
					Refresh(false);
		...
		else if (Category == TitleCategory.Veteran && vetTitles != null && vetTitles.Count > 0)
		{
			...
			else if (TitleSelected >= 0 && TitleSelected < vetTitles.Count)
			{
				...
				AddHtmlLocalized(480, 275, 80, 16, 1011046, 0xFFFF, false, false); // APPLY
				AddCallbackButton(445, 275, 4005, 4007, 799, GumpButtonType.Reply, 0, _ =>
				{
					AddHtmlLocalized(225, 315, 200, 16, 1115036, 0xFFFF, false, false); // TITLE APPLIED
	...
	public void CheckPage(ref int index, ref int page, int perpage = 9)
	{
		...
		AddCallbackButton(380, 275, 4005, 4007, 0, GumpButtonType.Page, page + 1, _ => { });
		...
		AddCallbackButton(225, 275, 4014, 4016, 0, GumpButtonType.Page, page - 1, _ => { });
	}
	...
	private bool HasTitle()
	{
		...
		{
			TitleType.PaperdollSuffix => User.PaperdollSkillTitle != null || User.CurrentChampTitle != null,
			TitleType.OverheadName => User.OverheadTitle != null || User.DisplayGuildAbbr,
			TitleType.SubTitles => User.SubtitleSkillTitle != null || User.SelectedTitle > -1 ||
			                       User.CurrentVeteranTitle > 0 || User.DisplayGuildTitle,
			_ => false
		};
	}
	...
	private string GetChampionTitle()
	{
		...
		if (TitleSelected == 295)
		{
			...
			if (v > 0)
				str =
					$": {Titles.HarrowerTitles[Math.Min(Titles.HarrowerTitles.Length, info.Harrower) - 1]} of Evil";
		}
		else
		{
	...
	private object GetChampInfo()
	{
		...
		return index switch
		{
			0 => 1115047 // This title is gained from battling against the minions of Semidar.
			,
			1 => 1115046 // This title is gained from battling against the minions of Mephitis.
			,
			2 => 1115044 // This title is gained from battling against the minions of Rikktor.
			,
			3 => 1115045 // This title is gained from battling against the minions of Lord Oaks.
			,
			4 => 1115048 // This title is gained from battling against the minions of Barracoon the Piper.
			,
			5 => 1115049 // This title is gained from battling against the minions of Neira the Necromancer.
			,
			6 => 1115050 // This title is gained from battling against the minions of Serado the Awakened.
			,
			7 => 1115052 // This title is gained from battling against the minions of Twaulo of the Glade.
			,
			8 => 1115051 // This title is gained from battling against the minions of Ilhenir the Stained.
			,
			12 => 1115054 // This title is gained from battling against the minions of the Abyssal Inferno.
			,
			13 => 1115053 // This title is gained from battling against the minions of the Primeval Lich.
			,
			14 =>
				"This title is gained from battling against the minions of the Dragon Turtle." // Dragon Turtle, no cliloc???
			,
			295 => 1115055 // This title is gained from battling against the Harrower and the Champions.
			,
			_ => "Unknown Champion"
		};
	}

	private static object GetRewardTitleInfo(object title)
	{
		if (title is not int id)
		{
		...
		return id switch
		{
			>= 1152739 and <= 1152893 or 1154017 => 1152893,// This title is gained through city loyalty.
			>= 1151739 and <= 1151747 or 1155481 or 1154505 => id + 1,
			>= 1157181 and <= 1157203 => 1157180,// This title is obtained from turning in Bulk Order Deeds.
			>= 1156985 and <= 1156987 => 1156984,// This title is obtained from the Halloween Treasures of the Kotl City Event.	
			>= 1152068 and <= 1152073 => 1152075,// This is a reward title given for your valorous fights in arenas.
			>= 1155600 and <= 1155603 => 1155601,// This title is obtained from the Halloween Treasures of Doom Event.
			>= 1157649 and <= 1157651 => 1157648,// This title is obtained from the Halloween Treasures of the Sorcerer's Dungeon Event.	
			_ => id switch
			{
				1073235 or 1073236 or 1073237 or 1073238 or 1073239 => 1115115,// This is a reward title given for contributing to the Vesper Museum.
				1073201 or 1073202 or 1073203 or 1073204 or 1073205 or 1073206 => 1115116,// This is a reward title given for contributing to the Britannia Royal Zoo.
				1073341 or 1073342 or 1073343 or 1073344 or 1073345 => 1115117,// This is a reward title given for contributing to the Britain Public Library.
				1156477 => 1156476,// This title is obtained by completing the Valley of One Quest in the Valley of Eodon.
				1155727 => 1155728,// This title is obtained from the Huntmaster's Challenge at the Ranger's Guild in Skara Brae.
				1156318 => 1156319,// This is a reward title given for completing Shadowguard.
				1157594 => 1157595,// This title is obtained by completing the "Discovering Animal Training" quest.
				1158140 => 1158141,// This title is obtained by completing the Huntmaster's Challenge Quest.
				1158090 => 1158087,// This title is obtained by completing the Paladins of Trinsic Quest.
				1158161 => 1158162,// This title is obtained by completing the Righting Wrong Quest.	
				1158389 => 1158248,// This title is obtained by completing the Treasure Chase Quest.
				1158278 => 1158279,// This title is obtained by completing the "A Forced Sacrifice" quest.
				1158303 => 1158324,// This title is obtained by completing the "Whispering with Wisps" quest.
				1154505 => 1154506,// This is a reward title given for completing the Exploring the Deep Quest.
				1158523 => 1158537,// This title is obtained by discovering a constellation using the telescope.
				_ => "This reward title has no desciption.",
			},
		};
	}

	private static bool IsCityTitle(int id)
	{
		return id is >= 1152739 and <= 1152893 || id is >= 1151739 and <= 1151747 or 1155481 or 1154017;
	}

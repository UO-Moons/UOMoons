...
public class MaginciaLottoGump : Gump
{
	...
	private const int BlueColor = 0x00BFFF;
	private const int LabelColor = 0xFFFFFF;
	private const int EntryColor = 0xE9967A;
	...
	public MaginciaLottoGump(Mobile from, MaginciaHousingPlot plot) : base(75, 75)
	{
		...
		int totalTicketsSold = plot.Participants.Values.Sum();
		...
		AddHtml(170, 50, 100, 16, $"<BASEFONT COLOR=#{LabelColor:X6}>{plot.Map}</BASEFONT>", false, false);
		...
		AddHtml(170, 95, 100, 16, $"<BASEFONT COLOR=#{EntryColor:X6}>{totalTicketsSold}</BASEFONT>", false, false);
		...
		AddHtml(170, 160, 100, 20, $"<BASEFONT COLOR=#{EntryColor:X6}>{ticketsBought}</BASEFONT>", false, false);
		...
		AddHtml(170, 225, 100, 20,
			$"<BASEFONT COLOR=#{EntryColor:X6}>{plot.LottoPrice:###,###,###}</BASEFONT>", false, false);
	...
	{
		bool prime = _plot.IsPrimeSpot;

		if (info.ButtonID == 0)
			return;

		if ((prime && !_plot.CanPurchaseLottoTicket(_from)) || !_plot.LottoOngoing)
			return;

		int pricePer = _plot.LottoPrice;
		int total = pricePer;
		int toBuy = 1;
		...
		if (Banker.Withdraw(_from, total))
		{
			MaginciaLottoSystem.GoldSink += total;
			_from.SendLocalizedMessage(1150480, $"{toBuy.ToString()}\t{pricePer.ToString()}\t{total.ToString()}"); // Purchase of ~1_COUNT~ ticket(s) at ~2_PRICE~gp each costs a total of ~3_TOTAL~. The funds have been withdrawn from your bank box and your ticket purchase has been recorded.
			_plot.PurchaseLottoTicket(_from, toBuy);
		}
		else
			_from.SendLocalizedMessage(1150479, $"{toBuy.ToString()}\t{pricePer.ToString()}\t{total.ToString()}"); // Purchase of ~1_COUNT~ ticket(s) at ~2_PRICE~gp each costs a total of ~3_TOTAL~. You do not have the required funds in your bank box to make the purchase.
	}
}

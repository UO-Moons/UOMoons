...
public class DistillationGump : Gump
{
	...
	private readonly DistillationContext _context;
	private readonly CraftDefinition _def;

	public DistillationGump(Mobile from) : base(35, 35)
	{
		from.CloseGump(typeof(DistillationGump));

		_context = DistillationSystem.GetContext(from);

		Group group = _context.LastGroup;
		Liquor liquor = _context.LastLiquor;

		if (liquor == Liquor.None)
			liquor = DistillationSystem.GetFirstLiquor(group);

		_def = DistillationSystem.GetDefinition(liquor, group);

		if (_def == null)
			return;

		//if (_def.Liquor != liquor)
		liquor = _def.Liquor;
		...
		y = 80;
		for (int i = 0; i < _def.Ingredients.Length; i++)
		{
			Type type = _def.Ingredients[i];
			int amt = _def.Amounts[i];
			bool strong = _context.MakeStrong;

			if (i == 0 && type == typeof(Yeast))
			{
				for (int j = 0; j < amt; j++)
				{
					Yeast yeast = _context.SelectedYeast[j];
			...
			else
			{
				...
				if (amount > total)
					amount = total;
				AddHtmlLocalized(295, y, 200, 20, 1150733,
					$"#{_def.Labels[i]}\t{amount}/{total}", LabelColor, false, false); // ~1_NAME~ : ~2_NUMBER~
			}
		...
		AddHtmlLocalized(295, 290, 200, 20, _context.MakeStrong ? 1150730 : 1150729, LabelColor, false, false); // Double Distillation - Standard Distillation

		AddButton(255, 316, 4005, 4007, 5, GumpButtonType.Reply, 0);
		AddHtmlLocalized(295, 320, 200, 20, _context.Mark ? 1150731 : 1150732, LabelColor, false, false); // Mark Distiller Name - Do Not Mark

		AddButton(15, 395, 4005, 4007, 6, GumpButtonType.Reply, 0);
		AddHtmlLocalized(55, 395, 200, 20, 1150733, $"Label\t{_context.Label ?? "None"}", LabelColor, false, false); // ~1_NAME~ : ~2_NUMBER~
	...
	public override void OnResponse(NetState state, RelayInfo info)
	{
		...
		switch (info.ButtonID)
		{
			...
			case 1: // Select Wheat Based
				_context.LastGroup = Group.WheatBased;
				break;
			case 2: // Select Water Based
				_context.LastGroup = Group.WaterBased;
				break;
			case 3: // Select Other
				_context.LastGroup = Group.Other;
				break;
			case 4: // Distillation Strength
				_context.MakeStrong = !_context.MakeStrong;
				break;
			case 5: // Mark Option
				_context.Mark = !_context.Mark;
				break;
			case 6: // Label
				from.Prompt = new LabelPrompt(_context);
			...
			case 7: // Execute Distillation
				from.Target = new DistillationTarget(from, _context, _def);
			...
			default:
			{
				if (info.ButtonID < 2000) // Select Liquor Type
				{
					...
					if (sel >= 0 && sel < DistillationSystem.CraftDefs.Count)
					{
						...
						_context.LastLiquor = def.Liquor;
					}
				}
				else				      // Select Yeast
				{
					...
					if (_def.Ingredients[0] == typeof(Yeast) && _def.Amounts.Length > 0)
					{
						int amt = _def.Amounts[0];

						if (sel >= 0 && sel < amt)
						{
							from.Target = new YeastSelectionTarget(_context, sel);
	...
	public class DistillationTarget : Target
	{
		...
		protected override void OnTarget(Mobile from, object targeted)
		{
			if (targeted is LiquorBarrel barrel)
			{
				if (barrel.IsChildOf(from.Backpack))
				{
					...
					{
						double perc = _context.MakeStrong ? 2 : 1;
						...
						if (HasTotal(from, perc))
						{
							...
							for (int i = 0; i < _context.SelectedYeast.Length; i++)
							{
								Yeast yeast = _context.SelectedYeast[i];
							...
							if (chance > Utility.Random(100))
							{
								...
								_context.ClearYeasts();
								from.SendLocalizedMessage(1150772); // You succeed at your distillation attempt.

								Mobile marker = _context.Mark ? from : null;
								from.PlaySound(0x2D6);

								barrel.BeginDistillation(_def.Liquor, _def.MaturationDuration, _context.Label, _context.MakeStrong, marker);
							}
							else
							{
								from.PlaySound(0x2D6);
		...
		private bool HasTotal(Mobile from, double percentage)
		{
			for (int i = 0; i < _def.Ingredients.Length; i++)
			{
				...
				else
				{
					...
					if (GetAmount(from, type, _def.Liquor) < toConsume)
						return false;
				}
			}
		...
		private void ConsumeTotal(Mobile from, double percentage)
		{
			...
			for (int i = 0; i < _def.Ingredients.Length; i++)
			{
				...
				{
					toConsume = (int)(toConsume * percentage);
					BeverageType bType = _def.Liquor == Liquor.Brandy ? BeverageType.Wine : BeverageType.Water;
				...
				else if (type == typeof(PewterBowlOfCorn))
				{
					...
					//var totalWooden = pack.GetAmount(typeof(WoodenBowlOfCorn));
	...
	private class YeastSelectionTarget : Target
	{
		...
		protected override void OnTarget(Mobile from, object targeted)
		{
			if (targeted is Yeast yeast)
			{
				...
				{
					if (!_context.YeastInUse(yeast))
					{
						_context.SelectedYeast[_index] = yeast;
		...
		protected override void OnTargetCancel(Mobile from, TargetCancelType cancelType)
		{
			if (_context.SelectedYeast[_index] != null)
			{
				_context.SelectedYeast[_index] = null;

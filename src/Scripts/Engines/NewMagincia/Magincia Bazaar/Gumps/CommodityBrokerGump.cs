...
namespace Server.Engines.NewMagincia
{
    ...
    {
        private readonly CommodityBroker _broker;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                default:
	                return;
                case 1:
                    from.SendGump(new BazaarInformationGump(0, 1150637, new CommodityBrokerGump(_broker, from)));
                    return;
                ...
                case 5: // ADD TO INVENTORY
                    ...
                    from.Target = new InternalTarget(_broker);
                    from.SendGump(new CommodityTargetGump(_broker));
                    return;
                case 6: // VIEW INVENTORY / REMOVE ITEMS
                    ...
                    from.SendGump(new ViewInventoryGump(_broker));
                    return;
                case 7: // SET PRICES AND LIMITS
                    ...
                    from.SendGump(new SetPricesAndLimitsGump(_broker));
                    return;
            }

            from.SendGump(new CommodityBrokerGump(_broker, from));
        }

        public class InternalTarget : Target
        {
            private readonly CommodityBroker _broker;
            private bool _hasPickedCommodity;
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is Item item and (ICommodity or CommodityDeed))
                {
	                if (item.IsChildOf(from.Backpack))
                    {
                        if (item is CommodityDeed {Commodity: null})
                        {
                            from.SendLocalizedMessage(1150222); // That item is not a commodity that a broker can trade.
                        }
                        else if (_broker.CommodityEntries.Count < CommodityBroker.MaxEntries)
                        {
                            if (!_broker.TryAddBrokerEntry(item, from))    // Adds new entry									// Already has an entry, adds item to inventory for selling
                                _broker.AddInventory(from, item);          // or adds to the stock

                            _hasPickedCommodity = true;
                        }
                        else
                        {
                ...
                from.Target = new InternalTarget(_broker, _hasPickedCommodity);
            }
    ...
    public class CommodityTargetGump : BaseBazaarGump
    {
            ...
            : base(520, 520)
        {
            ...
            if (broker.Plot.ShopName is {Length: > 0})
            {
                AddHtml(10, 37, 500, 18, Color(FormatStallName(broker.Plot.ShopName), BlueColor), false, false);
            }
            else
            {
                AddHtmlLocalized(10, 37, 500, 18, 1114513, "#1150314", BlueColor16, false, false); // This Shop Has No Name
    ...
    public class SetPricesAndLimitsGump : BaseBazaarGump
    {
            ...
            : base(520, 520)
        {
            ...
            {
                int col = i == _index ? YellowColor : OrangeColor;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1)
            {
                from.SendGump(new BazaarInformationGump(0, 1150644, new SetPricesAndLimitsGump(_broker, _index, _page)));
                return;
            }

            if (info.ButtonID == 400) // back page
            {
	            from.SendGump(new SetPricesAndLimitsGump(_broker, -1, _page - 1));
	            return;
            }
            ...
            {
	            if (_index >= 0 && _index < _broker.CommodityEntries.Count)
	            {
		            CommodityBrokerEntry entry = _broker.CommodityEntries[_index];
		            ...
		            if (buyLmt is < 0 or > 60000 || sellLmt is < 0 or > 60000)
            ...
            else
            {
	            ...
	            if (id >= 0 && id < _broker.CommodityEntries.Count)
		            _index = id;
            }

            from.SendGump(new SetPricesAndLimitsGump(_broker, _index, _page));
        }
    }
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                ...
                case 1: // Withdraw
                    if (_index >= 0 && _index < _broker.CommodityEntries.Count)
                    {
                        ...
                        if (amount <= 0 || amount > entry.Stock)
                            ...
                            _broker.WithdrawInventory(from, amount, entry);
                    }
                    else
                        from.SendLocalizedMessage(1150642); // You did not select a commodity.
                    break;
                ...
                case 3: // Remove Entry
                    if (_index >= 0 && _index < _broker.CommodityEntries.Count)
                    {
                        from.SendGump(new ConfirmRemoveEntryGump(_broker, _index));
                        return;
                    }
                    ...
                    break;
                case 400:
                    from.SendGump(new ViewInventoryGump(_broker, -1, _page - 1));
                    return;
                case 401:
                    from.SendGump(new ViewInventoryGump(_broker, -1, _page + 1));
                    return;
                ...
                default:
                    {
                        if (info.ButtonID < 1500) // SELECT ENTRY
                        {
                            ...
                            if (id >= 0 && id < _broker.CommodityEntries.Count)
                            {
                                from.SendGump(new ViewInventoryGump(_broker, id, _page));
                                return;
                            }
                        }
                        else if (info.ButtonID < 2000) // UP
                        {
                            ...
                            if (id > 0 && id < _broker.CommodityEntries.Count)
                            {
                                CommodityBrokerEntry entry = _broker.CommodityEntries[id];
                                int idx = _broker.CommodityEntries.IndexOf(entry);

                                _broker.CommodityEntries.Remove(entry);
                                _broker.CommodityEntries.Insert(idx - 1, entry);
                        ...
                        else // DOWN
                        {
                            ...
                            if (id >= 0 && id < _broker.CommodityEntries.Count - 1)
                            {
                                CommodityBrokerEntry entry = _broker.CommodityEntries[id];
                                int idx = _broker.CommodityEntries.IndexOf(entry);
            ...
            from.SendGump(new ViewInventoryGump(_broker, _index));
        }
    }
    ...
    {
        private readonly CommodityBroker _broker;
        private readonly List<CommodityBrokerEntry> _entries;
        private bool _buy;
        private readonly int _page;
        private int _index;
        ...
        {
            _broker = broker;
            _entries = broker.CommodityEntries;
            _buy = buy;
            _page = page;
            _index = index;
            ...
            if (_broker.Plot.ShopName is {Length: > 0})
            {
                AddHtml(10, 37, 640, 18, Color(FormatStallName(_broker.Plot.ShopName), BlueColor), false, false);
            }
            else
            {
            ...
            else
            {
                ...
                if (_entries.Count != 0)
                {
                    ...
                    for (int i = start; i < _entries.Count && /*index < perPage*/ count <= perPage; i++)
                    {
                        CommodityBrokerEntry entry = _entries[i];
                    ...
                    if (_entries.Count - start > perPage) // forward
                    {
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                case 0: return;
                case 999:
                    from.SendGump(new CommodityInventoryGump(_broker, _index, _buy, _page, 0));
                    return;
                case 1:
                    from.SendGump(new CommodityInventoryGump(_broker, _index, _buy, _page, 1150654));
                    return;
                case 400:
                    from.SendGump(new CommodityInventoryGump(_broker, -1, _buy, _page - 1, 0));
                    return;
                case 401:
                    from.SendGump(new CommodityInventoryGump(_broker, -1, _buy, _page + 1, 0));
                    return;
                case 500: // TRADE
                    {
                        for (int i = 0; i < _entries.Count; i++)
                        {
                            ...
                            {
                                if (_broker.CommodityEntries.Contains(_entries[i]))
                                {
                                    _buy = true;
                                    _index = i;
                                }
                        ...
                        {
                            CommodityBrokerEntry entry = _entries[_index];

                            if (!_broker.CommodityEntries.Contains(entry))
                            {
                                from.SendGump(new CommodityInventoryGump(_broker, _index, _buy, _page, 1150244)); // Transaction no longer available
                                return;
                            }
                            ...
                            if (amount > 0)
                            {
	                            if (_buy && Banker.GetBalance(from) < entry.SellPricePer * amount)
                                {
                                    ...
                                    */
                                    from.SendGump(new CommodityInventoryGump(_broker, _index, _buy, _page, 1150252));
                                    return;
                                }

	                            if (!_buy && !_broker.SellCommodityControl(from, entry, amount))
	                            {
                                    ...
                                    */
		                            from.SendGump(new CommodityInventoryGump(_broker, _index, _buy, _page, 1150667));
		                            return;
	                            }

	                            if (_buy && entry.PlayerCanBuy(amount))
	                            {
		                            from.SendGump(new ConfirmBuyCommodityGump(_broker, amount, entry, true, new CommodityInventoryGump(_broker, _index, _buy, _page, 0)));
		                            return;
	                            }

	                            if (!_buy && entry.PlayerCanSell(amount))
	                            {
		                            from.SendGump(new ConfirmBuyCommodityGump(_broker, amount, entry, false, new CommodityInventoryGump(_broker, _index, _buy, _page, 0)));
		                            return;
	                            }

	                            from.SendGump(new CommodityInventoryGump(_broker, _index, _buy, _page, 1150215)); // You have entered an invalid value, or a non-numeric value. Please try again.
	                            return;
                            }
                        ...
                        from.SendGump(new CommodityInventoryGump(_broker, _index, _buy, _page, 1150642)); // You did not select a commodity.
                        return;
                    }
            }
        }
    ...
    {
        private readonly CommodityBroker _broker;
        private readonly CommodityBrokerEntry _entry;
        private readonly int _index;
        ...
        public ConfirmRemoveEntryGump(CommodityBroker broker, int index)
        {
            ...
            if (index >= 0 && index < _broker.CommodityEntries.Count)
                _entry = _broker.CommodityEntries[index];
        }

        public override void Confirm(Mobile from)
        {
            if (_entry != null && _broker.CommodityEntries.Contains(_entry))
                _broker.RemoveEntry(from, _entry);
    ...
    public class ConfirmBuyCommodityGump : BaseBazaarGump
    {
            ...
            : base(660, 520)
        {
            ...
            if (buy)
            {
                ...
                AddHtmlLocalized(230, 193, 420, 18, _broker.GetLabelId(entry), DarkGreenColor16, false, false);
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            {
	            case 1: //BUY
                    _broker.TryBuyCommodity(from, _entry, _amount);
                    break;
                case 2: //SELL
                    _broker.TrySellCommodity(from, _entry, _amount);
                    break;
                case 3: //MAIN MENU
                    from.SendGump(_gump);
                    break;

...
{
	private DateTime _nextFee;

	[CommandProperty(AccessLevel.GameMaster)]
	public MaginciaBazaarPlot Plot { get; set; }
	...
	public BaseBazaarBroker(MaginciaBazaarPlot plot) : base(AIType.AI_Vendor, FightMode.None, 2, 1, 0.5, 2)
	{
		Plot = plot;
		BankBalance = 0;
		_nextFee = DateTime.UtcNow + TimeSpan.FromHours(23);
		InitBody();
		InitOutfit();
	...
	{
		if (Plot != null && MaginciaBazaar.Instance != null)
			MaginciaBazaar.Instance.AddInventoryToWarehouse(Plot.Owner, this);

		if (Plot != null) Plot.Merchant = null;
	...
	{
		if (_nextFee < DateTime.UtcNow)
		{
			BankBalance -= GetWeeklyFee() / 7;
	...
	{
		if (BankBalance < amount || !Banker.Deposit(from, amount))
	...
	public override void Serialize(GenericWriter writer)
	{
		...
		writer.Write(BankBalance);
		writer.Write(_nextFee);
	}
	...
	public override void Deserialize(GenericReader reader)
	{
		...
		BankBalance = reader.ReadInt();
		_nextFee = reader.ReadDateTime();
	}
}

...
namespace Server.Items
{
    ...
    {
	    private const int MaxStables = 3;

	    public override int LabelNumber => 1112570;  // a chicken coop

	    private readonly Dictionary<Mobile, List<BaseCreature>> m_Stored = new();
        ...
        private class StableTarget : Target
        {
            ...
            {
                if (targeted is ChickenLizard or Chicken or BattleChickenLizard)
        ...
        {
	        return m_Stored.Values.Where(bcList => bcList != null).Sum(bcList => bcList.Count);
        }

        private void EndStable(Mobile from, BaseCreature pet)
        {
	        if (Deleted || !from.CheckAlive() || !CanUse() || !CheckAccess(from))
                return;
        ...
        private void Claim(Mobile from)
        {
            if (Deleted || !from.CheckAlive() || !m_Stored.ContainsKey(from))
                return;
        ...
        private bool CheckAccess(Mobile m)
        {
            ...
            return h != null && h.HasSecureAccess(m, Level);
        }
        ...
        {
	        if (!CheckAccess(e.Mobile) || !IsLockedDown) return;
	        switch (e.Handled)
	        {
		        case false when e.HasKeyword(0x0008):
			        e.Handled = true;
			        BeginStable(e.Mobile);
			        break;
		        case false when e.HasKeyword(0x0009):
		        {
			        e.Handled = true;
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            for (int i = 0; i < c; i++)
            {
                ...
                for (int j = 0; j < count; j++)
                {
                    ...
                    if (chicken is not BaseCreature bc)
	                    continue;

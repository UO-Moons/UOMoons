...
namespace Server.Items
{
    ...
    public sealed class ChickenLizardEgg : Item
    {
	    private static bool CanMutate => true;
        ...
        private Dryness Dryness
        {
            get
            {
                int v = (int)Stage - m_WaterLevel;

                return v switch
                {
	                >= 2 when m_WaterLevel == 0 => Dryness.Dehydrated,
	                >= 2 => Dryness.Parched,
	                >= 1 => Dryness.Dry,
	                _ => Dryness.Moist
                };
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        private bool IsBattleChicken { get; set; }
            ...
            get
            {
                ...
                switch (Stage)
                {
	                case EggStage.Mature:
		                c = IsBattleChicken ? 1112468 : 1112467;
		                break;
	                case EggStage.Burnt:
		                c = 1112466;
		                break;
        ...
        public override bool DropToItem(Mobile from, Item target, Point3D p)
        {
            ...
            if (check && Parent is not Incubator && m_Incubating)
                Incubating = false;
        ...
        public void Pour(Mobile from, BaseBeverage bev)
        {
            if (!bev.IsEmpty && bev.Pourable && bev.Content == BeverageType.Water && bev.ValidateUse(from, false))
            {
                ...
                else if (m_WaterLevel < (int)Stage)
                {
                    bev.Quantity--;
                ...
                case EggStage.Mature:
                    {
                        ...
                        double chance = Dryness switch
                        {
	                        Dryness.Dry => .05,
	                        Dryness.Parched => .01,
	                        Dryness.Dehydrated => 0,
	                        _ => .10
                        };

                        if (CanMutate && chance >= Utility.RandomDouble())
                        {
                            IsBattleChicken = true;
        ...
        private void TryHatchEgg(Mobile from)
        {
            if (Stage == EggStage.Mature)
                OnHatch(from);
            else
        ...
        private void OnHatch(Mobile from)
        {
            BaseCreature bc;
        ...
        private void CrumbleEgg(Mobile from)
        {
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            IncubationStart = reader.ReadDateTime();
            ...
            IsBattleChicken = reader.ReadBool();
        }
    }
}

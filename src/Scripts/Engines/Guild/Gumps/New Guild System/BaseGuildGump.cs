...
namespace Server.Guilds
{
	public abstract class BaseGuildGump : Gump
	{
		...
		{
			return !(m.Deleted || g.Disbanded || m is not PlayerMobile || (m.AccessLevel < AccessLevel.GameMaster && g.Leader != m));
		}

		public static bool IsMember(Mobile m, Guild g)
		{
			return !(m.Deleted || g.Disbanded || m is not PlayerMobile || (m.AccessLevel < AccessLevel.GameMaster && !g.IsMember(m)));
		}

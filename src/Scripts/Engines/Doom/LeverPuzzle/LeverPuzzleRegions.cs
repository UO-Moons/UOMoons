...
public class LampRoomRegion : BaseRegion
{
	...
	public LampRoomRegion(LeverPuzzleController controller)
		: base(null, Map.Malas, Region.Find(LeverPuzzleController.LrEnter, Map.Malas), LeverPuzzleController.LrRect)
	{
...
{
	private readonly LeverPuzzleController _controller;
	public Mobile MOccupant;

	[CommandProperty(AccessLevel.GameMaster)]
	public Mobile Occupant => MOccupant is {Alive: true} ? MOccupant : null;
	...
	public override void OnEnter(Mobile m)
	{
		if (m != null && MOccupant == null && m is PlayerMobile && m.Alive)
			MOccupant = m;
	}

	public override void OnExit(Mobile m)
	{
		if (m != null && m == MOccupant)
			MOccupant = null;
	}
}

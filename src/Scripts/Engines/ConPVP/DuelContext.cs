...
namespace Server.Engines.ConPVP
{
	...
	public class DuelContext
	{
		...
		public bool CheckItemEquip(Mobile from, Item item)
		{
			...
			{
				if ((armor.ProtectionLevel > ArmorProtectionLevel.Regular && !Ruleset.GetOption("Armor", "Magical")) || (!Core.AOS && armor.Resource != armor.DefaultResource && !Ruleset.GetOption("Armor", "Colored")) || (armor is BaseShield && !Ruleset.GetOption("Armor", "Shields")))
				{
					return false;
				}
			}
		...
		{
			if (mob.Spell is Spells.Spell spell)
				spell.Disturb(Spells.DisturbType.Kill);

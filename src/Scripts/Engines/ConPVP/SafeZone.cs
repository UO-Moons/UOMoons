...
public class SafeZone : GuardedRegion
{
	...
	{
		return from.IsStaff() && base.AllowHousing(from, p);
	}
	...
	public override bool OnMoveInto(Mobile m, Direction d, Point3D newLocation, Point3D oldLocation)
	{
		...
		if (pm == null && m is BaseCreature {Summoned: true} bc) pm = bc.SummonMaster as PlayerMobile;

		if (pm is {DuelContext.StartedBeginCountdown: true})
			return true;

...
public class Ladder
{
	...
	{
		(m_Entries[newIdx], m_Entries[idx]) = (m_Entries[idx], m_Entries[newIdx]);
	...
	public void UpdateEntry(LadderEntry entry)
	{
		...
		if (index >= 0 && index < m_Entries.Count)
		{
			...
			while ((index - 1) >= 0 && (_ = entry.CompareTo(m_Entries[index - 1])) < 0)
				index = Swap(index, index - 1);

			while ((index + 1) < m_Entries.Count && (_ = entry.CompareTo(m_Entries[index + 1])) > 0)
				index = Swap(index, index + 1);
		}
	}

...
{
	private static RulesetLayout _root;
	...
	public static RulesetLayout Root
	{
		get
		{
			if (_root == null)
			{
				...
				_root = new("Rules", (RulesetLayout[])entries.ToArray(typeof(RulesetLayout)));
				_root.ComputeOffsets();
				...
				if (!Core.AOS)
				{
					...
					_root.Defaults = new[] { m5X, m7X, s7X };
				}
				else
				{
					#region Standard All Skills

					Ruleset all = new(_root)
					{
					...
					_root.Defaults = new[] { all };
					#endregion
				}

				// Set up flavors

				Ruleset pots = new(_root)
				{
					Title = "Potions"
				};
				...
				_root.Flavors = new[] { pots, para, fields, area, summons };
			}
	...
	public RulesetLayout FindByTitle(string title)
	{
		...
		return Children.Select(t => t.FindByTitle(title)).FirstOrDefault(layout => layout != null);
	}
	...
	public string FindByIndex(int index)
	{
		...
		return Children.Select(t => t.FindByIndex(index)).FirstOrDefault(opt => opt != null);
	}

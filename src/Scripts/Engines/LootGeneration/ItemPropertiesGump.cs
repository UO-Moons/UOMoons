...
public class ItemPropertiesGump : BaseGump
{
	...
	private PropFilter Filter { get; set; }
	private ItemType TypeFilter { get; set; }
	private List<ItemPropertyInfo> Infos { get; set; }
	...
	private void LoadTypeInfo(ItemType type, ItemPropertyInfo info, int scale, int x, int y)
	{
		...
		if (typeInfo != null)
		{
			AddLabel(x, y, TypeFilter == type ? 0x9E : 0,
				$"{typeInfo.StandardMax}-{typeInfo.LootMax}[{(typeInfo.Scale > 1 ? typeInfo.Scale.ToString() : scale.ToString())}]");
		}
		else
		{
			AddLabel(x, y, 0, "N/A");
	...
	public override void OnResponse(RelayInfo info)
	{
		switch (info.ButtonID)
		{
			...
			case < 10:
				TypeFilter = info.ButtonID switch
				{
					2 => ItemType.Melee,
					3 => ItemType.Ranged,
					4 => ItemType.Armor,
...
{
	private ItemPropertyInfo Info { get; }
		...
		foreach (object i in Enum.GetValues(typeof(ItemType)))
		{
			...
			if (typeInfo != null)
			{
				...
				if (typeInfo.PowerfulLootRange != null)
				{
					...
					for (var j = 0; j < typeInfo.PowerfulLootRange.Length; j++)
					{
						var v = typeInfo.PowerfulLootRange[j];

...
public class TownHousesGump : GumpPlusLight
{
	...
	protected override void BuildGump()
	{
		...
		AddHtml(0, y += 20, width,
			"<CENTER>" + (_cListPage == ListPage.Town ? "TownHouses" : "Houses") + " Count: " + list.Count);
		AddHtml(0, y += 25, width, "<CENTER>TownHouses / Houses");
		AddButton(width / 2 - 100, y + 3, _cListPage == ListPage.Town ? 0x939 : 0x2716, "Page", Page, ListPage.Town);
		AddButton(width / 2 + 90, y + 3, _cListPage == ListPage.House ? 0x939 : 0x2716, "Page", Page, ListPage.House);

		y += 5;

		for (int i = _cPage * pp; i < (_cPage + 1) * pp && i < list.Count; ++i)
		{
			if (_cListPage == ListPage.Town)
			{

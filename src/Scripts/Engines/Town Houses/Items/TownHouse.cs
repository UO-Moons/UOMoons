...
public class TownHouse : VersionHouse
{
	...
	private Item _cHanger;
	private readonly List<Sector> _cSectors = new();

	public TownHouseSign ForSaleSign { get; private set; }

	public Item Hanger
	{
		get => _cHanger ??= new Item(0xB98) { Movable = false, Location = Sign.Location, Map = Sign.Map };
		set => _cHanger = value;
	}
	...
	public void InitSectorDefinition()
	{
		...
		foreach (Sector sector in _cSectors)
		{
		...
		for (int x = minX; x < maxX; ++x)
		{
			...
			{
				if (!_cSectors.Contains(Map.GetSector(new Point2D(x, y))))
				{
					_cSectors.Add(Map.GetSector(new Point2D(x, y)));
				}
			}
		}
	...
	public override void OnLocationChange(Point3D oldLocation)
	{
		...
		if (_cHanger != null)
		{
			_cHanger.Location = Sign.Location;
		}
	}

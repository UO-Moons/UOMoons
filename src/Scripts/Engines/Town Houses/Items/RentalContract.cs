...
{
	private Mobile _cRentalClient;
	public BaseHouse ParentHouse { get; private set; }
	public Mobile RentalClient { get => _cRentalClient; set { _cRentalClient = value; InvalidateProperties(); } }
	...
	public override void ValidateOwnership()
	{
		...
		if (_cRentalClient != null && (ParentHouse == null || ParentHouse.Deleted))
		{
			Delete();
			return;
		}

		if (_cRentalClient != null && !Owned)
		{
			Delete();
			return;
		}
	...
	{
		if (ParentHouse == null || RentalMaster == null || _cRentalClient == null)
		{
			return;
		}

		RentalMaster.SendMessage("You have begun to use lockdowns reserved for {0}, and their rental unit will collapse in {1}.", _cRentalClient.Name, Math.Round((DemolishTime - DateTime.UtcNow).TotalHours, 2));
		_cRentalClient.SendMessage("Alert your land lord, {0}, they are using storage reserved for you.  They have violated the rental agreement, which will end in {1} if nothing is done.", RentalMaster.Name, Math.Round((DemolishTime - DateTime.UtcNow).TotalHours, 2));
	}
	...
	{
		if (RentalMaster == null || _cRentalClient == null)
		{
			return;
		}
		...
		RentalMaster.SendMessage("The bank has transfered your rent from {0}.", _cRentalClient.Name);
	}
	...
	{
		if (_cRentalClient != null)
		{
			list.Add("a house rental contract with " + _cRentalClient.Name);
		}
		else if (Completed)
		{
	...
	public override void Delete()
	{
		...
		if (!Owned && !ParentHouse.IsFriend(_cRentalClient))
		{
			if (_cRentalClient != null && RentalMaster != null)
			{
				...
				_cRentalClient.SendMessage("You have ended your rental agreement with {0}.  Because your access was revoked, your last payment is refunded.", _cRentalClient.Name);
			}

			DepositTo(_cRentalClient);
		}
		else if (Owned)
		{
			if (_cRentalClient != null && RentalMaster != null)
			{
				_cRentalClient.SendMessage("{0} has ended your rental agreement.  Since they broke the contract, your are refunded the last payment.", RentalMaster.Name);
				RentalMaster.SendMessage("You have ended your rental agreement with {0}.  They will be refunded their last payment.", _cRentalClient.Name);
			}
		...
		else
		{
			if (_cRentalClient != null && RentalMaster != null)
			{
				RentalMaster.SendMessage("{0} has ended your rental agreement.", _cRentalClient.Name);
				_cRentalClient.SendMessage("You have ended your rental agreement with {0}.", RentalMaster.Name);
			}
	...
	public override void Deserialize(GenericReader reader)
	{
		...
		_cRentalClient = reader.ReadMobile();

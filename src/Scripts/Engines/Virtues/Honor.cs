...
public class HonorContext
{
	...
	public void OnTargetDamaged(Mobile from, int amount)
	{
		...
		if (from == Source)
		{
			if (Target.CanSee(Source) && Target.InLOS(Source) && (Source.InRange(Target, 1)
			                                                      || (Source.Location == _initialLocation && Source.Map == _initialMap)))
			{
				_honorDamage += amount;
			}
			else
	...
	public void OnTargetKilled()
	{
		...
		double dGain = targetFame / 100 * (_honorDamage / _totalDamage);    //Initial honor gain is 100th of the monsters honor

		if (Math.Abs(_honorDamage - _totalDamage) < targetFame && _firstHit == FirstHit.Granted)
		...
		{
			Source.SendLocalizedMessage(gainedPath ? 1063226 : 1063225);
		}
	}

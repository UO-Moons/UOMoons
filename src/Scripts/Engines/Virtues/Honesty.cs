...
public static class HonestyVirtue
{
	...
	private static readonly string[] m_Regions =
		{"Britain", "Minoc", "Magincia", "Trinsic", "Jhelom", "Moonglow", "Skara Brae", "Yew"};

	private const TileFlag Filter = TileFlag.Wet | TileFlag.Roof | TileFlag.Impassable;
	...
	static HonestyVirtue()
	{
		...
		m_Items = new List<Item>(MaxGeneration);
	}

	public static void Initialize()
	{
		EventSink.OnItemDeleted += OnItemDeleted;
		...
		if (Enabled)
		{
			m_Items.AddRange(World.Items.Values.Where(item => item.HonestyItem));
	...
	private static void OnItemDeleted(Item item)
	{
		if (IsHonestyItem(item))
		{
			m_Items.Remove(item);
		}
	}
	...
	{
		m_Items.RemoveAll(ItemFlags.GetTaken);
	}
	...
	*/
	...
	private static void GenerateHonestyItems()
	{
		...
		if (UseSpawnArea)
		{
			if (_feluccaArea == null)
			{
				...
				_feluccaArea = SpawnArea.Instantiate(Map.Felucca.DefaultRegion, Filter, ValidateSpawnPoint, true);
			...
			if (_trammelArea == null && TrammelGeneration)
			{
				...
				_trammelArea = SpawnArea.Instantiate(Map.Trammel.DefaultRegion, Filter, ValidateSpawnPoint, true);
		...
		try
		{
			var count = MaxGeneration - m_Items.Count;
	...
	public static void AssignOwner(HonestyItemSocket socket)
	{
		if (socket != null)
		{
			socket.HonestyRegion = m_Regions[Utility.Random(m_Regions.Length)];
			...
			if (!string.IsNullOrWhiteSpace(socket.HonestyRegion) && BaseVendor.AllVendors.Count >= 10)
			{
				...
				socket.HonestyOwner = matchedVendors.Count > 0 ? matchedVendors[Utility.Random(matchedVendors.Count)] : BaseVendor.AllVendors[Utility.Random(BaseVendor.AllVendors.Count)];
			}
		}
	}
	...
	{
		foreach (var loc in m_ChestLocations)
		{
	...
	private static readonly Point3D[] m_ChestLocations =
	{
		new(1809, 2825, 0), new(1323, 3768, 0), new(3784, 2247, 20), new(591, 2144, 0),
		new(1648, 1603, 20), new(2509, 544, 0), new(4463, 1156, 0), new(650, 824, 0)
	};
}

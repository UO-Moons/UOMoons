...
public class JusticeVirtue
{
	...
	{
		if (map is not {MapID: < 2})
			return 0;

		if (loc.X < 5120)
			return 0;

		return loc.Y < 2304 ? 1 : 2;
	}
...
public class AcceptProtectorGump : Gump
{
	...
	private readonly PlayerMobile _protection;

	public AcceptProtectorGump(PlayerMobile protector, PlayerMobile protection) : base(150, 50)
	{
	...
	public override void OnResponse(NetState sender, RelayInfo info)
	{
		if (info.ButtonID == 2)
		{
			...
			if (okay)
				JusticeVirtue.OnVirtueAccepted(_protector, _protection);
			else
				JusticeVirtue.OnVirtueRejected(_protector, _protection);
		}
	}
}

...
public class ChatSystem
{
	...
	public static void OpenChatWindowRequest(NetState state, PacketReader pvSrc)
	{
		...
		else
		{
			...
			{
				if ((from Account checkAccount in Accounts.GetAccounts() select checkAccount.GetTag("ChatName") into existingName where existingName != null select existingName.Trim()).Any(existingName => Insensitive.Equals(existingName, chatName)))
				{
